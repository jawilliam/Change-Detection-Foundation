...
{
    /// <summary>
    /// Implements the euclidean distance based similarity.
    /// </summary>
    [Serializable]
    public class EuclideanDistanceSimetric<T> : VectorSimetric<T> where T : IEquatable<T>
    {
        /// <summary>
        ...
        public override double GetSimilarity(IEnumerable<T> firstSequence, IEnumerable<T> secondSequence, out double? distance)
        {
            ...
            var uniqueTokens = firstSeq.Union(secondSeq, this.Comparer).ToList();
            ...
            //var total = Math.Sqrt(firstComponents.Select((t, i) => Math.Pow(Math.Max(t, secondComponents[i]), 2)).Sum());
            distance = Math.Sqrt(firstComponents.Select((t, i) => Math.Pow(t - secondComponents[i], 2)).Sum());

            return 1 / (1 + distance.Value);
        }
    }
}