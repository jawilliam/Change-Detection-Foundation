...
{
    /// <summary>
    /// Implements the overlap coefficient based similarity.
    /// </summary>
    [Serializable]
    public class OverlapCoefficientSimetric<T> : SimpleSimetric<T> where T : IEquatable<T>
    {
        /// <summary>
        ...
        public override double GetSimilarity(IEnumerable<T> firstSequence, IEnumerable<T> secondSequence)
        {
            ...
            var uniqueTokens = firstSeq.Union(secondSeq, this.Comparer).ToArray();
            var firstComponents = uniqueTokens.Select(t => firstSeq.Count(ft => this.Comparer.Equals(t, ft))).ToArray();
            var secondComponents = uniqueTokens.Select(t => secondSeq.Count(ft => this.Comparer.Equals(t, ft))).ToArray();

            double overlap = firstComponents.Select((t, i) => Math.Min(t, secondComponents[i])).Sum();

            return overlap == 0 ? 0 : overlap / Math.Min(firstSeq.Length, secondSeq.Length);
        }
