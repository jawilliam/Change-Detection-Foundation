...
{
    /// <summary>
    /// Implements the jaccard similarity metric.
    /// </summary>
    /// <typeparam name="T">The concrete type of the elements to be compared.</typeparam>
    [Serializable]
    public class JaccardSimetric<T> : VectorSimetric<T> where T : IEquatable<T>
    {
        /// <summary>
        ...
        public override double GetSimilarity(IEnumerable<T> firstSequence, IEnumerable<T> secondSequence)
        {
            ...
            if (uniqueTokens.Any())
            {
                double pq = firstComponents.Select((d, i) => d * secondComponents[i]).Sum();
                double p2 = firstComponents.Select(d => Math.Pow(d, 2)).Sum();
                double q2 = secondComponents.Select(d => Math.Pow(d, 2)).Sum();
