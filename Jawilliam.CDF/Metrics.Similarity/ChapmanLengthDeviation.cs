...
namespace Jawilliam.CDF.Metrics.Similarity
{
    ...
    /// </summary>
    [Serializable]
    public class ChapmanLengthDeviation<T> : SimpleSimetric<T> where T : IEquatable<T>
    {
        /// <summary>
        ...
        public override double GetSimilarity(IEnumerable<T> firstSequence, IEnumerable<T> secondSequence, out double? distance)
        {
            ...
            double min = Math.Min(firstSeq.Length, secondSeq.Length);
            return min/Math.Max(firstSeq.Length, secondSeq.Length);
        }
    }
}