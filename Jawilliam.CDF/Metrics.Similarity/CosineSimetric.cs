...
namespace Jawilliam.CDF.Metrics.Similarity
{
    ...
    [Serializable]
    public class CosineSimetric<T> : VectorSimetric<T> where T : IEquatable<T>
    {
        /// <summary>
        ...
        public override double GetSimilarity(IEnumerable<T> firstSequence, IEnumerable<T> secondSequence)
        {
            ...
            if (uniqueTokens.Any())
            {
                ...
                var aa = firstComponents.Select(d => Math.Pow(d, 2)).Sum();
                var bb = secondComponents.Select(d => Math.Pow(d, 2)).Sum();

                return ab == 0 ? 0 : ab / Math.Sqrt(aa * bb);
            }
