...
namespace Jawilliam.CDF.Approach.Criterions.Simetric
{
    ...
    public class OverlapCoefficientSimetric<T> : SimpleSimetric<T> where T : IEquatable<T>
    {
        ...
        public override double GetSimilarity(IEnumerable<T> firstSequence, IEnumerable<T> secondSequence)
        {
            ...
            var firstComponents = uniqueTokens.Select(t => firstSeq.Count(ft => this.AreEqual(t, ft))).ToArray();
            var secondComponents = uniqueTokens.Select(t => secondSeq.Count(ft => this.AreEqual(t, ft))).ToArray();
