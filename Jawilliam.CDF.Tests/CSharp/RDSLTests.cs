...
namespace Jawilliam.CDF.Tests.CSharp
{
    ...
    {
        [TestMethod]
        public void GuidelinesForTopologicallyRelevantProperties()
        {
        ...
        [TestMethod]
        public void GuidelinesForReadOnlyTypes()
        {
            var rdsl = CDF.XObjects.RDSL.Syntax.Load(@"..\..\..\Jawilliam.CDF.CSharp\RDSL.xml");
            var concreteTypes = rdsl.Nodes.Type.Where(n => !n.@abstract).ToArray();
            ...
            var readonlyTypesWithJustReadOnlyProperties = typesWithJustReadOnlyProperties.Except(readonlyTypes).ToArray();
            var readonlyTypesWithJustReadOnlyProperties2 = readonlyTypes.Except(typesWithJustReadOnlyProperties).ToArray();
            //var expressionButNonTopologicalProperties = expressionProperties.Except(readonlyTypes).ToArray();
            ;
            ...
            //// All operator property is topologically relevant.
            //var operatorButNonTopologicalProperties = operatorProperties.Except(readonlyTypes).ToArray();
            //Assert.AreEqual(operatorButNonTopologicalProperties.Length, 0);
            ...
            //// All identifier token property is topologically relevant.
            //var identifierTokenButNonTopologicalProperties = identifierTokenProperties.Except(readonlyTypes).ToArray();
            //Assert.AreEqual(identifierTokenButNonTopologicalProperties.Length, 0);
            ...
            //                             (p.name == "File" && t.name == "LoadDirectiveTriviaSyntax")
            //                       select new { Type = t, Property = p }).ToArray();
            //var otherPropertiesButNonTopologicalProperties = otherProperties.Except(readonlyTypes).ToArray();
            //Assert.AreEqual(otherPropertiesButNonTopologicalProperties.Length, 0);

            //var topologicalButNonExpressionProperties = readonlyTypes.Except(
            //    expressionProperties.Union(operatorProperties).Union(identifierTokenProperties).Union(otherProperties)
            //).ToArray();
            //Assert.AreEqual(topologicalButNonExpressionProperties.Length, 0);
        }
