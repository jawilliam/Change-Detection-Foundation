namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void ParenthesizedLambdaExpressionServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<ParenthesizedLambdaExpressionSyntax> nodes);
         ...
         public void ParenthesizedLambdaExpressionServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<ParenthesizedLambdaExpressionSyntax> nodes = null;
    	    string expectedLabel = null;
    	    ParenthesizedLambdaExpressionServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var ArrowTokenLabel = Enum.GetName(typeof(SyntaxKind), node.ArrowToken.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == ArrowTokenLabel).Attribute("mLb").Value == ArrowTokenLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == ArrowTokenLabel).GtID())
    				.Attribute("mId").Value == mFullElement.GtID());
