namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
         /// <summary>
         /// Provides the element revision pair(s) to test in <see cref="ParenthesizedLambdaExpressionServiceProvider_RoslynMLDeltaExpander_OK"/>.
         /// </summary>
    	 ...
    	 /// <summary>
         /// Tests expansion logic for <see cref="ParenthesizedLambdaExpressionSyntax"/>.
         /// </summary>
    	 [TestMethod]
         public void ParenthesizedLambdaExpressionServiceProvider_RoslynMLDeltaExpander_OK()
         {
    		var converter = new CDF.CSharp.RoslynML.RoslynML();
    		var selector = new CDF.CSharp.RoslynML.RoslynMLPruneSelector();
    		int id = 0; 
    
    	    IEnumerable<(ParenthesizedLambdaExpressionSyntax, ParenthesizedLambdaExpressionSyntax)> nodeRevisionPairs = null;
    	    string oExpectedLabel = null, mExpectedLabel = null;
    	    ParenthesizedLambdaExpressionServiceProvider_RoslynMLDeltaExpander_DataToTest(ref nodeRevisionPairs);
    		foreach(((ParenthesizedLambdaExpressionSyntax Original, ParenthesizedLambdaExpressionSyntax Modified) nodeRevisionPair, Action<RoslynML, XElement> defoliate) in nodeRevisionPairs
    			.SelectMany(n => new List<((ParenthesizedLambdaExpressionSyntax, ParenthesizedLambdaExpressionSyntax), Action<RoslynML, XElement>)>
