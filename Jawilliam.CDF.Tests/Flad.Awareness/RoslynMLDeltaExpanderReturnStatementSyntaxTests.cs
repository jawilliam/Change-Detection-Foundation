namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void ReturnStatementServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<ReturnStatementSyntax> nodes);
         ...
         public void ReturnStatementServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<ReturnStatementSyntax> nodes = null;
    	    string expectedLabel = null;
    	    ReturnStatementServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var ReturnKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.ReturnKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == ReturnKeywordLabel).Attribute("mLb").Value == ReturnKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == ReturnKeywordLabel).GtID())
