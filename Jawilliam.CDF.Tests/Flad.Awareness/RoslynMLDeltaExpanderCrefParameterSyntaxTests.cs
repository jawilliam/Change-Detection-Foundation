namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
         /// <summary>
         /// Provides the element revision pair(s) to test in <see cref="CrefParameterServiceProvider_RoslynMLDeltaExpander_OK"/>.
         /// </summary>
    	 ...
    	 /// <summary>
         /// Tests expansion logic for <see cref="CrefParameterSyntax"/>.
         /// </summary>
    	 [TestMethod]
         public void CrefParameterServiceProvider_RoslynMLDeltaExpander_OK()
         {
    		var converter = new CDF.CSharp.RoslynML.RoslynML();
    		var selector = new CDF.CSharp.RoslynML.RoslynMLPruneSelector();
    		int id = 0; 
    
    	    IEnumerable<(CrefParameterSyntax, CrefParameterSyntax)> nodeRevisionPairs = null;
    	    string oExpectedLabel = null, mExpectedLabel = null;
    	    CrefParameterServiceProvider_RoslynMLDeltaExpander_DataToTest(ref nodeRevisionPairs);
    		foreach(((CrefParameterSyntax Original, CrefParameterSyntax Modified) nodeRevisionPair, Action<RoslynML, XElement> defoliate) in nodeRevisionPairs
    			.SelectMany(n => new List<((CrefParameterSyntax, CrefParameterSyntax), Action<RoslynML, XElement>)>
    				{ (n, (r, n1) => { }), (n, (r, n1) => r.Defoliate(n1)) }))
    		{
    			...
    			var oRefKindKeywordLabel = nodeRevisionPair.Original.RefKindKeyword == null || nodeRevisionPair.Original.RefKindKeyword.Kind() == SyntaxKind.None
    				? null 
    				: Enum.GetName(typeof(SyntaxKind), nodeRevisionPair.Original.RefKindKeyword.Kind());
    			...
    			if(mFullElement1.Elements().Any(e => e.Label() == mRefKindKeywordLabel))
    				mFullElement1.Elements().Single(e => e.Label() == mRefKindKeywordLabel).Value = "v0";
    			   ...
    			   mFullElement.Elements().Any(e => e.Attribute("part").Value == "RefKindKeyword"))
    			{
    				...
    				Assert.IsTrue(expander.FullDelta.Actions
    					.Single(a => a.Name.LocalName == "Update" && a.Attribute("eLb").Value == oRefKindKeywordLabel)
