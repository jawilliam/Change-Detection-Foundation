namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void CrefParameterServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<CrefParameterSyntax> nodes);
         ...
         public void CrefParameterServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<CrefParameterSyntax> nodes = null;
    	    string expectedLabel = null;
    	    CrefParameterServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			existingProperties = oFullElement.Elements().Any(e => e.Name.LocalName == "RefKindKeyword") ? existingProperties : (existingProperties - 1);
    			...
    			var RefKindKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.RefKindKeyword.Kind());
    			if(oFullElement.Elements().Any(e => e.Name.LocalName == "RefKindKeyword"))
    			{
    				Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == RefKindKeywordLabel).Attribute("mLb").Value == RefKindKeywordLabel);
    				Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    					oFullElement.Elements().Single(e => e.Attribute("oLb").Value == RefKindKeywordLabel).GtID())
    					.Attribute("mId").Value == mFullElement.GtID());
    			}
    			else
    			{
    				Assert.IsFalse(expander.FullDelta.Matches.Any(m => m.Attribute("oLb").Value == RefKindKeywordLabel);
    			}
