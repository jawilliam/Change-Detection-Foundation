namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void BreakStatementServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<BreakStatementSyntax> nodes);
         ...
         public void BreakStatementServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<BreakStatementSyntax> nodes = null;
    	    string expectedLabel = null;
    	    BreakStatementServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var BreakKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.BreakKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == BreakKeywordLabel).Attribute("mLb").Value == BreakKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == BreakKeywordLabel).GtID())
