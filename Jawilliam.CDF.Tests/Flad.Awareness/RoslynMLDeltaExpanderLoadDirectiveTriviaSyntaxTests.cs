namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void LoadDirectiveTriviaServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<LoadDirectiveTriviaSyntax> nodes);
         ...
         public void LoadDirectiveTriviaServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<LoadDirectiveTriviaSyntax> nodes = null;
    	    string expectedLabel = null;
    	    LoadDirectiveTriviaServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var LoadKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.LoadKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == LoadKeywordLabel).Attribute("mLb").Value == LoadKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == LoadKeywordLabel).GtID())
    				.Attribute("mId").Value == mFullElement.GtID());
