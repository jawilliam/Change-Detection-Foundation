namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void LockStatementServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<LockStatementSyntax> nodes);
         ...
         public void LockStatementServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<LockStatementSyntax> nodes = null;
    	    string expectedLabel = null;
    	    LockStatementServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var LockKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.LockKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == LockKeywordLabel).Attribute("mLb").Value == LockKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == LockKeywordLabel).GtID())
