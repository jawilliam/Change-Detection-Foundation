namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void RefValueExpressionServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<RefValueExpressionSyntax> nodes);
         ...
         public void RefValueExpressionServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<RefValueExpressionSyntax> nodes = null;
    	    string expectedLabel = null;
    	    RefValueExpressionServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var KeywordLabel = Enum.GetName(typeof(SyntaxKind), node.Keyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == KeywordLabel).Attribute("mLb").Value == KeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == KeywordLabel).GtID())
    			...
    			var CommaLabel = Enum.GetName(typeof(SyntaxKind), node.Comma.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == CommaLabel).Attribute("mLb").Value == CommaLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == CommaLabel).GtID())
    				.Attribute("mId").Value == mFullElement.GtID());
