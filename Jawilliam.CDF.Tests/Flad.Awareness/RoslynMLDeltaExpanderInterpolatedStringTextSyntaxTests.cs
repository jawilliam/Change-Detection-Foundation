namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void InterpolatedStringTextServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<InterpolatedStringTextSyntax> nodes);
         ...
         public void InterpolatedStringTextServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<InterpolatedStringTextSyntax> nodes = null;
    	    string expectedLabel = null;
    	    InterpolatedStringTextServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var TextTokenLabel = Enum.GetName(typeof(SyntaxKind), node.TextToken.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == TextTokenLabel).Attribute("mLb").Value == TextTokenLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == TextTokenLabel).GtID())
