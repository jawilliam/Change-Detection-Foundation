namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
         /// <summary>
         /// Provides the element revision pair(s) to test in <see cref="SwitchStatementServiceProvider_RoslynMLDeltaExpander_OK"/>.
         /// </summary>
    	 ...
    	 /// <summary>
         /// Tests expansion logic for <see cref="SwitchStatementSyntax"/>.
         /// </summary>
    	 [TestMethod]
         public void SwitchStatementServiceProvider_RoslynMLDeltaExpander_OK()
         {
    		var converter = new CDF.CSharp.RoslynML.RoslynML();
    		var selector = new CDF.CSharp.RoslynML.RoslynMLPruneSelector();
    		int id = 0; 
    
    	    IEnumerable<(SwitchStatementSyntax, SwitchStatementSyntax)> nodeRevisionPairs = null;
    	    string oExpectedLabel = null, mExpectedLabel = null;
    	    SwitchStatementServiceProvider_RoslynMLDeltaExpander_DataToTest(ref nodeRevisionPairs);
    		foreach(((SwitchStatementSyntax Original, SwitchStatementSyntax Modified) nodeRevisionPair, Action<RoslynML, XElement> defoliate) in nodeRevisionPairs
    			.SelectMany(n => new List<((SwitchStatementSyntax, SwitchStatementSyntax), Action<RoslynML, XElement>)>
    				{ (n, (r, n1) => { }), (n, (r, n1) => r.Defoliate(n1)) }))
    		{
    			...
    			var oSwitchKeywordLabel = Enum.GetName(typeof(SyntaxKind), nodeRevisionPair.Original.SwitchKeyword.Kind());
    			var mSwitchKeywordLabel = Enum.GetName(typeof(SyntaxKind), nodeRevisionPair.Modified.SwitchKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == oSwitchKeywordLabel)
    				.Attribute("mLb").Value == mSwitchKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Label() == oSwitchKeywordLabel).GtID())
    				.Attribute("mId").Value == 
    				mFullElement.Elements().Single(e => e.Label() == mSwitchKeywordLabel).GtID());
    			...
    			Assert.IsTrue(expander.FullDelta.Actions
    					.Single(a => a.Name.LocalName == "Insert" && a.Attribute("eLb").Value == mSwitchKeywordLabel)
    					.Attribute("pLb").Value == mExpectedLabel);
    			Assert.IsTrue(expander.FullDelta.Actions
    			...
    			Assert.IsTrue(expander.FullDelta.Actions
    				.Single(a => a.Name.LocalName == "Delete" && a.Attribute("eId").Value == 
    					oFullElement.Elements().Single(e => e.Label() == oSwitchKeywordLabel).GtID())
    				.Attribute("eLb").Value == oSwitchKeywordLabel);
    			...
    			if(mFullElement1.Elements().Any(e => e.Label() == mSwitchKeywordLabel))
    				mFullElement1.Elements().Single(e => e.Label() == mSwitchKeywordLabel).Value = "v0";
    			...
    			if(mFullElement1.Elements().Any(e => e.Label() == mOpenBraceTokenLabel))
    				mFullElement1.Elements().Single(e => e.Label() == mOpenBraceTokenLabel).Value = "v3";
    			
    			if(mFullElement1.Elements().Any(e => e.Label() == mCloseBraceTokenLabel))
    				mFullElement1.Elements().Single(e => e.Label() == mCloseBraceTokenLabel).Value = "v4";
    			...
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				...
    				mFullElement.Elements().Single(e => e.Label() == mSwitchKeywordLabel).GtID());
    			Assert.IsTrue(expander.FullDelta.Actions
    					.Single(a => a.Name.LocalName == "Update" && a.Attribute("eLb").Value == oSwitchKeywordLabel)
