namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void SwitchStatementServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<SwitchStatementSyntax> nodes);
         ...
         public void SwitchStatementServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<SwitchStatementSyntax> nodes = null;
    	    string expectedLabel = null;
    	    SwitchStatementServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var SwitchKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.SwitchKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == SwitchKeywordLabel).Attribute("mLb").Value == SwitchKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == SwitchKeywordLabel).GtID())
