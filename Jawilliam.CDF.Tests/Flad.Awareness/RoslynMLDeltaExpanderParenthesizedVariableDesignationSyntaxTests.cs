namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
         /// <summary>
         /// Provides the element revision pair(s) to test in <see cref="ParenthesizedVariableDesignationServiceProvider_RoslynMLDeltaExpander_OK"/>.
         /// </summary>
    	 ...
    	 /// <summary>
         /// Tests expansion logic for <see cref="ParenthesizedVariableDesignationSyntax"/>.
         /// </summary>
    	 [TestMethod]
         public void ParenthesizedVariableDesignationServiceProvider_RoslynMLDeltaExpander_OK()
         {
    		var converter = new CDF.CSharp.RoslynML.RoslynML();
    		var selector = new CDF.CSharp.RoslynML.RoslynMLPruneSelector();
    		int id = 0; 
    
    	    IEnumerable<(ParenthesizedVariableDesignationSyntax, ParenthesizedVariableDesignationSyntax)> nodeRevisionPairs = null;
    	    string oExpectedLabel = null, mExpectedLabel = null;
    	    ParenthesizedVariableDesignationServiceProvider_RoslynMLDeltaExpander_DataToTest(ref nodeRevisionPairs);
    		foreach(((ParenthesizedVariableDesignationSyntax Original, ParenthesizedVariableDesignationSyntax Modified) nodeRevisionPair, Action<RoslynML, XElement> defoliate) in nodeRevisionPairs
    			.SelectMany(n => new List<((ParenthesizedVariableDesignationSyntax, ParenthesizedVariableDesignationSyntax), Action<RoslynML, XElement>)>
    				{ (n, (r, n1) => { }), (n, (r, n1) => r.Defoliate(n1)) }))
    		{
    			...
    			if(mFullElement1.Elements().Any(e => e.Label() == mCloseParenTokenLabel))
    				mFullElement1.Elements().Single(e => e.Label() == mCloseParenTokenLabel).Value = "v1";
