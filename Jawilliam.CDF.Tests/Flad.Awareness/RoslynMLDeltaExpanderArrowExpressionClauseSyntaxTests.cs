namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
         /// <summary>
         /// Provides the element revision pair(s) to test in <see cref="ArrowExpressionClauseServiceProvider_RoslynMLDeltaExpander_OK"/>.
         /// </summary>
    	 ...
    	 /// <summary>
         /// Tests expansion logic for <see cref="ArrowExpressionClauseSyntax"/>.
         /// </summary>
    	 [TestMethod]
         public void ArrowExpressionClauseServiceProvider_RoslynMLDeltaExpander_OK()
         {
    		var converter = new CDF.CSharp.RoslynML.RoslynML();
    		var selector = new CDF.CSharp.RoslynML.RoslynMLPruneSelector();
    		int id = 0; 
    
    	    IEnumerable<(ArrowExpressionClauseSyntax, ArrowExpressionClauseSyntax)> nodeRevisionPairs = null;
    	    string oExpectedLabel = null, mExpectedLabel = null;
    	    ArrowExpressionClauseServiceProvider_RoslynMLDeltaExpander_DataToTest(ref nodeRevisionPairs);
    		foreach(((ArrowExpressionClauseSyntax Original, ArrowExpressionClauseSyntax Modified) nodeRevisionPair, Action<RoslynML, XElement> defoliate) in nodeRevisionPairs
    			.SelectMany(n => new List<((ArrowExpressionClauseSyntax, ArrowExpressionClauseSyntax), Action<RoslynML, XElement>)>
    				{ (n, (r, n1) => { }), (n, (r, n1) => r.Defoliate(n1)) }))
    		{
    			...
    			if(mFullElement1.Elements().Any(e => e.Label() == mArrowTokenLabel))
    				mFullElement1.Elements().Single(e => e.Label() == mArrowTokenLabel).Value = "v0";
