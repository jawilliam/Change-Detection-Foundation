namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void EndRegionDirectiveTriviaServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<EndRegionDirectiveTriviaSyntax> nodes);
         ...
         public void EndRegionDirectiveTriviaServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<EndRegionDirectiveTriviaSyntax> nodes = null;
    	    string expectedLabel = null;
    	    EndRegionDirectiveTriviaServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var EndRegionKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.EndRegionKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == EndRegionKeywordLabel).Attribute("mLb").Value == EndRegionKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == EndRegionKeywordLabel).GtID())
    				.Attribute("mId").Value == mFullElement.GtID());
