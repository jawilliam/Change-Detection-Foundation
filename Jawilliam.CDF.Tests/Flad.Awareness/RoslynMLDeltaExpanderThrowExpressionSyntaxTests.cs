namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void ThrowExpressionServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<ThrowExpressionSyntax> nodes);
         ...
         public void ThrowExpressionServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<ThrowExpressionSyntax> nodes = null;
    	    string expectedLabel = null;
    	    ThrowExpressionServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var ThrowKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.ThrowKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == ThrowKeywordLabel).Attribute("mLb").Value == ThrowKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == ThrowKeywordLabel).GtID())
