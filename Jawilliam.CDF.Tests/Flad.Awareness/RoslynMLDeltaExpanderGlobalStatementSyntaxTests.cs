namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
         /// <summary>
         /// Provides the element revision pair(s) to test in <see cref="GlobalStatementServiceProvider_RoslynMLDeltaExpander_OK"/>.
         /// </summary>
    	 ...
    	 /// <summary>
         /// Tests expansion logic for <see cref="GlobalStatementSyntax"/>.
         /// </summary>
    	 [TestMethod]
         public void GlobalStatementServiceProvider_RoslynMLDeltaExpander_OK()
         {
    		var converter = new CDF.CSharp.RoslynML.RoslynML();
    		var selector = new CDF.CSharp.RoslynML.RoslynMLPruneSelector();
    		int id = 0; 
    
    	    IEnumerable<(GlobalStatementSyntax, GlobalStatementSyntax)> nodeRevisionPairs = null;
    	    string oExpectedLabel = null, mExpectedLabel = null;
    	    GlobalStatementServiceProvider_RoslynMLDeltaExpander_DataToTest(ref nodeRevisionPairs);
    		foreach(((GlobalStatementSyntax Original, GlobalStatementSyntax Modified) nodeRevisionPair, Action<RoslynML, XElement> defoliate) in nodeRevisionPairs
    			.SelectMany(n => new List<((GlobalStatementSyntax, GlobalStatementSyntax), Action<RoslynML, XElement>)>
