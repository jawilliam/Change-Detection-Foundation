namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void AliasQualifiedNameServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<AliasQualifiedNameSyntax> nodes);
         ...
         public void AliasQualifiedNameServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<AliasQualifiedNameSyntax> nodes = null;
    	    string expectedLabel = null;
    	    AliasQualifiedNameServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			existingProperties = oFullElement.Elements().Any(e => e.Name.LocalName == "ColonColonToken") ? existingProperties : (existingProperties - 1);
    			...
    			var ColonColonTokenLabel = Enum.GetName(typeof(SyntaxKind), node.ColonColonToken.Kind());
    			if(oFullElement.Elements().Any(e => e.Name.LocalName == "ColonColonToken"))
    			{
    				Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == ColonColonTokenLabel).Attribute("mLb").Value == ColonColonTokenLabel);
    				Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    					oFullElement.Elements().Single(e => e.Attribute("oLb").Value == ColonColonTokenLabel).GtID())
    					.Attribute("mId").Value == mFullElement.GtID());
    			}
    			else
    			{
    				Assert.IsFalse(expander.FullDelta.Matches.Any(m => m.Attribute("oLb").Value == ColonColonTokenLabel);
    			}
