namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void PragmaWarningDirectiveTriviaServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<PragmaWarningDirectiveTriviaSyntax> nodes);
         ...
         public void PragmaWarningDirectiveTriviaServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<PragmaWarningDirectiveTriviaSyntax> nodes = null;
    	    string expectedLabel = null;
    	    PragmaWarningDirectiveTriviaServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var WarningKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.WarningKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == WarningKeywordLabel).Attribute("mLb").Value == WarningKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == WarningKeywordLabel).GtID())
    				.Attribute("mId").Value == mFullElement.GtID());
    
    			var DisableOrRestoreKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.DisableOrRestoreKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == DisableOrRestoreKeywordLabel).Attribute("mLb").Value == DisableOrRestoreKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == DisableOrRestoreKeywordLabel).GtID())
    				.Attribute("mId").Value == mFullElement.GtID());
