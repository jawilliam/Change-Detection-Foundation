namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
         /// <summary>
         /// Provides the element revision pair(s) to test in <see cref="PragmaWarningDirectiveTriviaServiceProvider_RoslynMLDeltaExpander_OK"/>.
         /// </summary>
    	 ...
    	 /// <summary>
         /// Tests expansion logic for <see cref="PragmaWarningDirectiveTriviaSyntax"/>.
         /// </summary>
    	 [TestMethod]
         public void PragmaWarningDirectiveTriviaServiceProvider_RoslynMLDeltaExpander_OK()
         {
    		var converter = new CDF.CSharp.RoslynML.RoslynML();
    		var selector = new CDF.CSharp.RoslynML.RoslynMLPruneSelector();
    		int id = 0; 
    
    	    IEnumerable<(PragmaWarningDirectiveTriviaSyntax, PragmaWarningDirectiveTriviaSyntax)> nodeRevisionPairs = null;
    	    string oExpectedLabel = null, mExpectedLabel = null;
    	    PragmaWarningDirectiveTriviaServiceProvider_RoslynMLDeltaExpander_DataToTest(ref nodeRevisionPairs);
    		foreach(((PragmaWarningDirectiveTriviaSyntax Original, PragmaWarningDirectiveTriviaSyntax Modified) nodeRevisionPair, Action<RoslynML, XElement> defoliate) in nodeRevisionPairs
    			.SelectMany(n => new List<((PragmaWarningDirectiveTriviaSyntax, PragmaWarningDirectiveTriviaSyntax), Action<RoslynML, XElement>)>
    				{ (n, (r, n1) => { }), (n, (r, n1) => r.Defoliate(n1)) }))
    		{
    			...
    			var oWarningKeywordLabel = Enum.GetName(typeof(SyntaxKind), nodeRevisionPair.Original.WarningKeyword.Kind());
    			var mWarningKeywordLabel = Enum.GetName(typeof(SyntaxKind), nodeRevisionPair.Modified.WarningKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == oWarningKeywordLabel)
    				.Attribute("mLb").Value == mWarningKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Label() == oWarningKeywordLabel).GtID())
    				.Attribute("mId").Value == 
    				mFullElement.Elements().Single(e => e.Label() == mWarningKeywordLabel).GtID());
    
    			var oDisableOrRestoreKeywordLabel = Enum.GetName(typeof(SyntaxKind), nodeRevisionPair.Original.DisableOrRestoreKeyword.Kind());
    			var mDisableOrRestoreKeywordLabel = Enum.GetName(typeof(SyntaxKind), nodeRevisionPair.Modified.DisableOrRestoreKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == oDisableOrRestoreKeywordLabel)
    				.Attribute("mLb").Value == mDisableOrRestoreKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Label() == oDisableOrRestoreKeywordLabel).GtID())
    				.Attribute("mId").Value == 
    				mFullElement.Elements().Single(e => e.Label() == mDisableOrRestoreKeywordLabel).GtID());
    			...
    			Assert.IsTrue(expander.FullDelta.Actions
    					.Single(a => a.Name.LocalName == "Insert" && a.Attribute("eLb").Value == mWarningKeywordLabel)
    					.Attribute("pLb").Value == mExpectedLabel);
    			Assert.IsTrue(expander.FullDelta.Actions
    				.Single(a => a.Name.LocalName == "Insert" && a.Attribute("eId").Value == 
    					mFullElement.Elements().Single(e => e.Label() == mWarningKeywordLabel).GtID())
    			...
    			Assert.IsTrue(expander.FullDelta.Actions
    					.Single(a => a.Name.LocalName == "Insert" && a.Attribute("eLb").Value == mDisableOrRestoreKeywordLabel)
    					.Attribute("pLb").Value == mExpectedLabel);
    			Assert.IsTrue(expander.FullDelta.Actions
    				.Single(a => a.Name.LocalName == "Insert" && a.Attribute("eId").Value == 
    					mFullElement.Elements().Single(e => e.Label() == mDisableOrRestoreKeywordLabel).GtID())
    			...
    			Assert.IsTrue(expander.FullDelta.Actions
    				.Single(a => a.Name.LocalName == "Delete" && a.Attribute("eId").Value == 
    					oFullElement.Elements().Single(e => e.Label() == oWarningKeywordLabel).GtID())
    				.Attribute("eLb").Value == oWarningKeywordLabel);
    
    			Assert.IsTrue(expander.FullDelta.Actions
    				.Single(a => a.Name.LocalName == "Delete" && a.Attribute("eId").Value == 
    					oFullElement.Elements().Single(e => e.Label() == oDisableOrRestoreKeywordLabel).GtID())
    				.Attribute("eLb").Value == oDisableOrRestoreKeywordLabel);
    			...
    			if(mFullElement1.Elements().Any(e => e.Label() == mWarningKeywordLabel))
    				mFullElement1.Elements().Single(e => e.Label() == mWarningKeywordLabel).Value = "v2";
    			
    			if(mFullElement1.Elements().Any(e => e.Label() == mDisableOrRestoreKeywordLabel))
    				mFullElement1.Elements().Single(e => e.Label() == mDisableOrRestoreKeywordLabel).Value = "v3";
    			
    			if(mFullElement1.Elements().Any(e => e.Label() == mEndOfDirectiveTokenLabel))
    				mFullElement1.Elements().Single(e => e.Label() == mEndOfDirectiveTokenLabel).Value = "v4";
    			...
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				...
    				mFullElement.Elements().Single(e => e.Label() == mWarningKeywordLabel).GtID());
    			Assert.IsTrue(expander.FullDelta.Actions
    					.Single(a => a.Name.LocalName == "Update" && a.Attribute("eLb").Value == oWarningKeywordLabel)
    			...
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				...
    				mFullElement.Elements().Single(e => e.Label() == mDisableOrRestoreKeywordLabel).GtID());
    			Assert.IsTrue(expander.FullDelta.Actions
    					.Single(a => a.Name.LocalName == "Update" && a.Attribute("eLb").Value == oDisableOrRestoreKeywordLabel)
