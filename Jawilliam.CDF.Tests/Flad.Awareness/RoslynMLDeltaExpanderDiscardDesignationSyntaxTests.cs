namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
         /// <summary>
         /// Provides the element revision pair(s) to test in <see cref="DiscardDesignationServiceProvider_RoslynMLDeltaExpander_OK"/>.
         /// </summary>
    	 ...
    	 /// <summary>
         /// Tests expansion logic for <see cref="DiscardDesignationSyntax"/>.
         /// </summary>
    	 [TestMethod]
         public void DiscardDesignationServiceProvider_RoslynMLDeltaExpander_OK()
         {
    		var converter = new CDF.CSharp.RoslynML.RoslynML();
    		var selector = new CDF.CSharp.RoslynML.RoslynMLPruneSelector();
    		int id = 0; 
    
    	    IEnumerable<(DiscardDesignationSyntax, DiscardDesignationSyntax)> nodeRevisionPairs = null;
    	    string oExpectedLabel = null, mExpectedLabel = null;
    	    DiscardDesignationServiceProvider_RoslynMLDeltaExpander_DataToTest(ref nodeRevisionPairs);
    		foreach(((DiscardDesignationSyntax Original, DiscardDesignationSyntax Modified) nodeRevisionPair, Action<RoslynML, XElement> defoliate) in nodeRevisionPairs
    			.SelectMany(n => new List<((DiscardDesignationSyntax, DiscardDesignationSyntax), Action<RoslynML, XElement>)>
    				{ (n, (r, n1) => { }), (n, (r, n1) => r.Defoliate(n1)) }))
    		{
    			...
    			var oUnderscoreTokenLabel = Enum.GetName(typeof(SyntaxKind), nodeRevisionPair.Original.UnderscoreToken.Kind());
    			var mUnderscoreTokenLabel = Enum.GetName(typeof(SyntaxKind), nodeRevisionPair.Modified.UnderscoreToken.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == oUnderscoreTokenLabel)
    				.Attribute("mLb").Value == mUnderscoreTokenLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Label() == oUnderscoreTokenLabel).GtID())
    				.Attribute("mId").Value == 
    				mFullElement.Elements().Single(e => e.Label() == mUnderscoreTokenLabel).GtID());
    			...
    			Assert.IsTrue(expander.FullDelta.Actions
    					.Single(a => a.Name.LocalName == "Insert" && a.Attribute("eLb").Value == mUnderscoreTokenLabel)
    					.Attribute("pLb").Value == mExpectedLabel);
    			Assert.IsTrue(expander.FullDelta.Actions
    			...
    			Assert.IsTrue(expander.FullDelta.Actions
    				.Single(a => a.Name.LocalName == "Delete" && a.Attribute("eId").Value == 
    					oFullElement.Elements().Single(e => e.Label() == oUnderscoreTokenLabel).GtID())
    				.Attribute("eLb").Value == oUnderscoreTokenLabel);
    			...
    			if(mFullElement1.Elements().Any(e => e.Label() == mUnderscoreTokenLabel))
    				mFullElement1.Elements().Single(e => e.Label() == mUnderscoreTokenLabel).Value = "v0";
    			...
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				...
    				mFullElement.Elements().Single(e => e.Label() == mUnderscoreTokenLabel).GtID());
    			Assert.IsTrue(expander.FullDelta.Actions
    					.Single(a => a.Name.LocalName == "Update" && a.Attribute("eLb").Value == oUnderscoreTokenLabel)
