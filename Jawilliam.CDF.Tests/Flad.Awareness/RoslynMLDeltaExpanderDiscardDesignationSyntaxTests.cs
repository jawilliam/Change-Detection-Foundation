namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void DiscardDesignationServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<DiscardDesignationSyntax> nodes);
         ...
         public void DiscardDesignationServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<DiscardDesignationSyntax> nodes = null;
    	    string expectedLabel = null;
    	    DiscardDesignationServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var UnderscoreTokenLabel = Enum.GetName(typeof(SyntaxKind), node.UnderscoreToken.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == UnderscoreTokenLabel).Attribute("mLb").Value == UnderscoreTokenLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == UnderscoreTokenLabel).GtID())
