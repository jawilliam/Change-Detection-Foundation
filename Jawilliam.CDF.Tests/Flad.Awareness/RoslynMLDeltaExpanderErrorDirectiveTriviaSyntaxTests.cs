namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void ErrorDirectiveTriviaServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<ErrorDirectiveTriviaSyntax> nodes);
         ...
         public void ErrorDirectiveTriviaServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<ErrorDirectiveTriviaSyntax> nodes = null;
    	    string expectedLabel = null;
    	    ErrorDirectiveTriviaServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var ErrorKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.ErrorKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == ErrorKeywordLabel).Attribute("mLb").Value == ErrorKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == ErrorKeywordLabel).GtID())
    				.Attribute("mId").Value == mFullElement.GtID());
