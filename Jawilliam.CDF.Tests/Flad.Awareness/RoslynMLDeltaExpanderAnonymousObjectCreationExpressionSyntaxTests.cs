namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
         /// <summary>
         /// Provides the element revision pair(s) to test in <see cref="AnonymousObjectCreationExpressionServiceProvider_RoslynMLDeltaExpander_OK"/>.
         /// </summary>
    	 ...
    	 /// <summary>
         /// Tests expansion logic for <see cref="AnonymousObjectCreationExpressionSyntax"/>.
         /// </summary>
    	 [TestMethod]
         public void AnonymousObjectCreationExpressionServiceProvider_RoslynMLDeltaExpander_OK()
         {
    		var converter = new CDF.CSharp.RoslynML.RoslynML();
    		var selector = new CDF.CSharp.RoslynML.RoslynMLPruneSelector();
    		int id = 0; 
    
    	    IEnumerable<(AnonymousObjectCreationExpressionSyntax, AnonymousObjectCreationExpressionSyntax)> nodeRevisionPairs = null;
    	    string oExpectedLabel = null, mExpectedLabel = null;
    	    AnonymousObjectCreationExpressionServiceProvider_RoslynMLDeltaExpander_DataToTest(ref nodeRevisionPairs);
    		foreach(((AnonymousObjectCreationExpressionSyntax Original, AnonymousObjectCreationExpressionSyntax Modified) nodeRevisionPair, Action<RoslynML, XElement> defoliate) in nodeRevisionPairs
    			.SelectMany(n => new List<((AnonymousObjectCreationExpressionSyntax, AnonymousObjectCreationExpressionSyntax), Action<RoslynML, XElement>)>
    				{ (n, (r, n1) => { }), (n, (r, n1) => r.Defoliate(n1)) }))
    		{
    			...
    			if(mFullElement1.Elements().Any(e => e.Label() == mOpenBraceTokenLabel))
    				mFullElement1.Elements().Single(e => e.Label() == mOpenBraceTokenLabel).Value = "v1";
    			
    			if(mFullElement1.Elements().Any(e => e.Label() == mCloseBraceTokenLabel))
    				mFullElement1.Elements().Single(e => e.Label() == mCloseBraceTokenLabel).Value = "v2";
