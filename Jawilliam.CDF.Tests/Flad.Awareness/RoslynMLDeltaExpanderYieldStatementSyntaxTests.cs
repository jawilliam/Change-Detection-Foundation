namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void YieldStatementServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<YieldStatementSyntax> nodes);
         ...
         public void YieldStatementServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<YieldStatementSyntax> nodes = null;
    	    string expectedLabel = null;
    	    YieldStatementServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var YieldKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.YieldKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == YieldKeywordLabel).Attribute("mLb").Value == YieldKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == YieldKeywordLabel).GtID())
    				.Attribute("mId").Value == mFullElement.GtID());
    
    			var ReturnOrBreakKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.ReturnOrBreakKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == ReturnOrBreakKeywordLabel).Attribute("mLb").Value == ReturnOrBreakKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == ReturnOrBreakKeywordLabel).GtID())
    				.Attribute("mId").Value == mFullElement.GtID());
