namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void ForEachVariableStatementServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<ForEachVariableStatementSyntax> nodes);
         ...
         public void ForEachVariableStatementServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<ForEachVariableStatementSyntax> nodes = null;
    	    string expectedLabel = null;
    	    ForEachVariableStatementServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var ForEachKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.ForEachKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == ForEachKeywordLabel).Attribute("mLb").Value == ForEachKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == ForEachKeywordLabel).GtID())
