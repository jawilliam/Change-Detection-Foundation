namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void TypeParameterServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<TypeParameterSyntax> nodes);
         ...
         public void TypeParameterServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<TypeParameterSyntax> nodes = null;
    	    string expectedLabel = null;
    	    TypeParameterServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			existingProperties = oFullElement.Elements().Any(e => e.Name.LocalName == "VarianceKeyword") ? existingProperties : (existingProperties - 1);
    			...
    			var VarianceKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.VarianceKeyword.Kind());
    			if(oFullElement.Elements().Any(e => e.Name.LocalName == "VarianceKeyword"))
    			{
    				Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == VarianceKeywordLabel).Attribute("mLb").Value == VarianceKeywordLabel);
    				Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    					oFullElement.Elements().Single(e => e.Attribute("oLb").Value == VarianceKeywordLabel).GtID())
    					.Attribute("mId").Value == mFullElement.GtID());
    			}
    			else
    			{
    				Assert.IsFalse(expander.FullDelta.Matches.Any(m => m.Attribute("oLb").Value == VarianceKeywordLabel);
    			}
