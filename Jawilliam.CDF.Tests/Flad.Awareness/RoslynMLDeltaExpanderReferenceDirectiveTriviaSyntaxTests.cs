namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
         /// <summary>
         /// Provides the element revision pair(s) to test in <see cref="ReferenceDirectiveTriviaServiceProvider_RoslynMLDeltaExpander_OK"/>.
         /// </summary>
    	 ...
    	 /// <summary>
         /// Tests expansion logic for <see cref="ReferenceDirectiveTriviaSyntax"/>.
         /// </summary>
    	 [TestMethod]
         public void ReferenceDirectiveTriviaServiceProvider_RoslynMLDeltaExpander_OK()
         {
    		var converter = new CDF.CSharp.RoslynML.RoslynML();
    		var selector = new CDF.CSharp.RoslynML.RoslynMLPruneSelector();
    		int id = 0; 
    
    	    IEnumerable<(ReferenceDirectiveTriviaSyntax, ReferenceDirectiveTriviaSyntax)> nodeRevisionPairs = null;
    	    string oExpectedLabel = null, mExpectedLabel = null;
    	    ReferenceDirectiveTriviaServiceProvider_RoslynMLDeltaExpander_DataToTest(ref nodeRevisionPairs);
    		foreach(((ReferenceDirectiveTriviaSyntax Original, ReferenceDirectiveTriviaSyntax Modified) nodeRevisionPair, Action<RoslynML, XElement> defoliate) in nodeRevisionPairs
    			.SelectMany(n => new List<((ReferenceDirectiveTriviaSyntax, ReferenceDirectiveTriviaSyntax), Action<RoslynML, XElement>)>
    				{ (n, (r, n1) => { }), (n, (r, n1) => r.Defoliate(n1)) }))
    		{
    			...
    			var oReferenceKeywordLabel = Enum.GetName(typeof(SyntaxKind), nodeRevisionPair.Original.ReferenceKeyword.Kind());
    			var mReferenceKeywordLabel = Enum.GetName(typeof(SyntaxKind), nodeRevisionPair.Modified.ReferenceKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == oReferenceKeywordLabel)
    				.Attribute("mLb").Value == mReferenceKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Label() == oReferenceKeywordLabel).GtID())
    				.Attribute("mId").Value == 
    				mFullElement.Elements().Single(e => e.Label() == mReferenceKeywordLabel).GtID());
    			...
    			Assert.IsTrue(expander.FullDelta.Actions
    					.Single(a => a.Name.LocalName == "Insert" && a.Attribute("eLb").Value == mReferenceKeywordLabel)
    					.Attribute("pLb").Value == mExpectedLabel);
    			Assert.IsTrue(expander.FullDelta.Actions
    				.Single(a => a.Name.LocalName == "Insert" && a.Attribute("eId").Value == 
    					mFullElement.Elements().Single(e => e.Label() == mReferenceKeywordLabel).GtID())
    			...
    			Assert.IsTrue(expander.FullDelta.Actions
    				.Single(a => a.Name.LocalName == "Delete" && a.Attribute("eId").Value == 
    					oFullElement.Elements().Single(e => e.Label() == oReferenceKeywordLabel).GtID())
    				.Attribute("eLb").Value == oReferenceKeywordLabel);
    			...
    			if(mFullElement1.Elements().Any(e => e.Label() == mReferenceKeywordLabel))
    				mFullElement1.Elements().Single(e => e.Label() == mReferenceKeywordLabel).Value = "v1";
    			
    			if(mFullElement1.Elements().Any(e => e.Label() == mFileLabel))
    				mFullElement1.Elements().Single(e => e.Label() == mFileLabel).Value = "v2";
    			
    			if(mFullElement1.Elements().Any(e => e.Label() == mEndOfDirectiveTokenLabel))
    				mFullElement1.Elements().Single(e => e.Label() == mEndOfDirectiveTokenLabel).Value = "v3";
    			...
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				...
    				mFullElement.Elements().Single(e => e.Label() == mReferenceKeywordLabel).GtID());
    			Assert.IsTrue(expander.FullDelta.Actions
    					.Single(a => a.Name.LocalName == "Update" && a.Attribute("eLb").Value == oReferenceKeywordLabel)
