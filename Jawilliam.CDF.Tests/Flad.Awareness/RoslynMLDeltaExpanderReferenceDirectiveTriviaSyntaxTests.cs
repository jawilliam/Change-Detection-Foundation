namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void ReferenceDirectiveTriviaServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<ReferenceDirectiveTriviaSyntax> nodes);
         ...
         public void ReferenceDirectiveTriviaServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<ReferenceDirectiveTriviaSyntax> nodes = null;
    	    string expectedLabel = null;
    	    ReferenceDirectiveTriviaServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var ReferenceKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.ReferenceKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == ReferenceKeywordLabel).Attribute("mLb").Value == ReferenceKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == ReferenceKeywordLabel).GtID())
    				.Attribute("mId").Value == mFullElement.GtID());
