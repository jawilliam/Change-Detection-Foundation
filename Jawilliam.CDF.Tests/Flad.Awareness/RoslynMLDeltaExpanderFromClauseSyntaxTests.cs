namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void FromClauseServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<FromClauseSyntax> nodes);
         ...
         public void FromClauseServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<FromClauseSyntax> nodes = null;
    	    string expectedLabel = null;
    	    FromClauseServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var FromKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.FromKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == FromKeywordLabel).Attribute("mLb").Value == FromKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == FromKeywordLabel).GtID())
