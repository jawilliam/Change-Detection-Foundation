namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
         /// <summary>
         /// Provides the element revision pair(s) to test in <see cref="LetClauseServiceProvider_RoslynMLDeltaExpander_OK"/>.
         /// </summary>
    	 ...
    	 /// <summary>
         /// Tests expansion logic for <see cref="LetClauseSyntax"/>.
         /// </summary>
    	 [TestMethod]
         public void LetClauseServiceProvider_RoslynMLDeltaExpander_OK()
         {
    		var converter = new CDF.CSharp.RoslynML.RoslynML();
    		var selector = new CDF.CSharp.RoslynML.RoslynMLPruneSelector();
    		int id = 0; 
    
    	    IEnumerable<(LetClauseSyntax, LetClauseSyntax)> nodeRevisionPairs = null;
    	    string oExpectedLabel = null, mExpectedLabel = null;
    	    LetClauseServiceProvider_RoslynMLDeltaExpander_DataToTest(ref nodeRevisionPairs);
    		foreach(((LetClauseSyntax Original, LetClauseSyntax Modified) nodeRevisionPair, Action<RoslynML, XElement> defoliate) in nodeRevisionPairs
    			.SelectMany(n => new List<((LetClauseSyntax, LetClauseSyntax), Action<RoslynML, XElement>)>
    				{ (n, (r, n1) => { }), (n, (r, n1) => r.Defoliate(n1)) }))
    		{
    			...
    			var oLetKeywordLabel = Enum.GetName(typeof(SyntaxKind), nodeRevisionPair.Original.LetKeyword.Kind());
    			var mLetKeywordLabel = Enum.GetName(typeof(SyntaxKind), nodeRevisionPair.Modified.LetKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == oLetKeywordLabel)
    				.Attribute("mLb").Value == mLetKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Label() == oLetKeywordLabel).GtID())
    				.Attribute("mId").Value == 
    				mFullElement.Elements().Single(e => e.Label() == mLetKeywordLabel).GtID());
    			...
    			Assert.IsTrue(expander.FullDelta.Actions
    					.Single(a => a.Name.LocalName == "Insert" && a.Attribute("eLb").Value == mLetKeywordLabel)
    					.Attribute("pLb").Value == mExpectedLabel);
    			Assert.IsTrue(expander.FullDelta.Actions
    			...
    			Assert.IsTrue(expander.FullDelta.Actions
    				.Single(a => a.Name.LocalName == "Delete" && a.Attribute("eId").Value == 
    					oFullElement.Elements().Single(e => e.Label() == oLetKeywordLabel).GtID())
    				.Attribute("eLb").Value == oLetKeywordLabel);
    			...
    			if(mFullElement1.Elements().Any(e => e.Label() == mLetKeywordLabel))
    				mFullElement1.Elements().Single(e => e.Label() == mLetKeywordLabel).Value = "v0";
    			...
    			if(mFullElement1.Elements().Any(e => e.Label() == mEqualsTokenLabel))
    				mFullElement1.Elements().Single(e => e.Label() == mEqualsTokenLabel).Value = "v2";
    			...
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				...
    				mFullElement.Elements().Single(e => e.Label() == mLetKeywordLabel).GtID());
    			Assert.IsTrue(expander.FullDelta.Actions
    					.Single(a => a.Name.LocalName == "Update" && a.Attribute("eLb").Value == oLetKeywordLabel)
