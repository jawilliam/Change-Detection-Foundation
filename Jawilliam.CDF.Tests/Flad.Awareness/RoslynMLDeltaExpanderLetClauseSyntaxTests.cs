namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void LetClauseServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<LetClauseSyntax> nodes);
         ...
         public void LetClauseServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<LetClauseSyntax> nodes = null;
    	    string expectedLabel = null;
    	    LetClauseServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var LetKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.LetKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == LetKeywordLabel).Attribute("mLb").Value == LetKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == LetKeywordLabel).GtID())
