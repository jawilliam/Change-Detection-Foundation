namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
         /// <summary>
         /// Provides the element revision pair(s) to test in <see cref="StackAllocArrayCreationExpressionServiceProvider_RoslynMLDeltaExpander_OK"/>.
         /// </summary>
    	 ...
    	 /// <summary>
         /// Tests expansion logic for <see cref="StackAllocArrayCreationExpressionSyntax"/>.
         /// </summary>
    	 [TestMethod]
         public void StackAllocArrayCreationExpressionServiceProvider_RoslynMLDeltaExpander_OK()
         {
    		var converter = new CDF.CSharp.RoslynML.RoslynML();
    		var selector = new CDF.CSharp.RoslynML.RoslynMLPruneSelector();
    		int id = 0; 
    
    	    IEnumerable<(StackAllocArrayCreationExpressionSyntax, StackAllocArrayCreationExpressionSyntax)> nodeRevisionPairs = null;
    	    string oExpectedLabel = null, mExpectedLabel = null;
    	    StackAllocArrayCreationExpressionServiceProvider_RoslynMLDeltaExpander_DataToTest(ref nodeRevisionPairs);
    		foreach(((StackAllocArrayCreationExpressionSyntax Original, StackAllocArrayCreationExpressionSyntax Modified) nodeRevisionPair, Action<RoslynML, XElement> defoliate) in nodeRevisionPairs
    			.SelectMany(n => new List<((StackAllocArrayCreationExpressionSyntax, StackAllocArrayCreationExpressionSyntax), Action<RoslynML, XElement>)>
    				{ (n, (r, n1) => { }), (n, (r, n1) => r.Defoliate(n1)) }))
    		{
    			...
    			var oStackAllocKeywordLabel = Enum.GetName(typeof(SyntaxKind), nodeRevisionPair.Original.StackAllocKeyword.Kind());
    			var mStackAllocKeywordLabel = Enum.GetName(typeof(SyntaxKind), nodeRevisionPair.Modified.StackAllocKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == oStackAllocKeywordLabel)
    				.Attribute("mLb").Value == mStackAllocKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Label() == oStackAllocKeywordLabel).GtID())
    				.Attribute("mId").Value == 
    				mFullElement.Elements().Single(e => e.Label() == mStackAllocKeywordLabel).GtID());
    			...
    			Assert.IsTrue(expander.FullDelta.Actions
    					.Single(a => a.Name.LocalName == "Insert" && a.Attribute("eLb").Value == mStackAllocKeywordLabel)
    					.Attribute("pLb").Value == mExpectedLabel);
    			Assert.IsTrue(expander.FullDelta.Actions
    			...
    			Assert.IsTrue(expander.FullDelta.Actions
    				.Single(a => a.Name.LocalName == "Delete" && a.Attribute("eId").Value == 
    					oFullElement.Elements().Single(e => e.Label() == oStackAllocKeywordLabel).GtID())
    				.Attribute("eLb").Value == oStackAllocKeywordLabel);
    			...
    			if(mFullElement1.Elements().Any(e => e.Label() == mStackAllocKeywordLabel))
    				mFullElement1.Elements().Single(e => e.Label() == mStackAllocKeywordLabel).Value = "v0";
    			...
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				...
    				mFullElement.Elements().Single(e => e.Label() == mStackAllocKeywordLabel).GtID());
    			Assert.IsTrue(expander.FullDelta.Actions
    					.Single(a => a.Name.LocalName == "Update" && a.Attribute("eLb").Value == oStackAllocKeywordLabel)
