namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void StackAllocArrayCreationExpressionServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<StackAllocArrayCreationExpressionSyntax> nodes);
         ...
         public void StackAllocArrayCreationExpressionServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<StackAllocArrayCreationExpressionSyntax> nodes = null;
    	    string expectedLabel = null;
    	    StackAllocArrayCreationExpressionServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var StackAllocKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.StackAllocKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == StackAllocKeywordLabel).Attribute("mLb").Value == StackAllocKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == StackAllocKeywordLabel).GtID())
