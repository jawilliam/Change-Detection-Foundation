namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void AnonymousMethodExpressionServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<AnonymousMethodExpressionSyntax> nodes);
         ...
         public void AnonymousMethodExpressionServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<AnonymousMethodExpressionSyntax> nodes = null;
    	    string expectedLabel = null;
    	    AnonymousMethodExpressionServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			existingProperties = oFullElement.Elements().Any(e => e.Name.LocalName == "AsyncKeyword") ? existingProperties : (existingProperties - 1);
    			...
    			var AsyncKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.AsyncKeyword.Kind());
    			if(oFullElement.Elements().Any(e => e.Name.LocalName == "AsyncKeyword"))
    			{
    				Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == AsyncKeywordLabel).Attribute("mLb").Value == AsyncKeywordLabel);
    				Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    					oFullElement.Elements().Single(e => e.Attribute("oLb").Value == AsyncKeywordLabel).GtID())
    					.Attribute("mId").Value == mFullElement.GtID());
    			}
    			else
    			{
    				Assert.IsFalse(expander.FullDelta.Matches.Any(m => m.Attribute("oLb").Value == AsyncKeywordLabel);
    			}
    
    			var DelegateKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.DelegateKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == DelegateKeywordLabel).Attribute("mLb").Value == DelegateKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == DelegateKeywordLabel).GtID())
    				.Attribute("mId").Value == mFullElement.GtID());
