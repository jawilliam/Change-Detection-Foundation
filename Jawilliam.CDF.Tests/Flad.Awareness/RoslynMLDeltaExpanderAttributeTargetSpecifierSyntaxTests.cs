namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
         /// <summary>
         /// Provides the element revision pair(s) to test in <see cref="AttributeTargetSpecifierServiceProvider_RoslynMLDeltaExpander_OK"/>.
         /// </summary>
    	 ...
    	 /// <summary>
         /// Tests expansion logic for <see cref="AttributeTargetSpecifierSyntax"/>.
         /// </summary>
    	 [TestMethod]
         public void AttributeTargetSpecifierServiceProvider_RoslynMLDeltaExpander_OK()
         {
    		var converter = new CDF.CSharp.RoslynML.RoslynML();
    		var selector = new CDF.CSharp.RoslynML.RoslynMLPruneSelector();
    		int id = 0; 
    
    	    IEnumerable<(AttributeTargetSpecifierSyntax, AttributeTargetSpecifierSyntax)> nodeRevisionPairs = null;
    	    string oExpectedLabel = null, mExpectedLabel = null;
    	    AttributeTargetSpecifierServiceProvider_RoslynMLDeltaExpander_DataToTest(ref nodeRevisionPairs);
    		foreach(((AttributeTargetSpecifierSyntax Original, AttributeTargetSpecifierSyntax Modified) nodeRevisionPair, Action<RoslynML, XElement> defoliate) in nodeRevisionPairs
    			.SelectMany(n => new List<((AttributeTargetSpecifierSyntax, AttributeTargetSpecifierSyntax), Action<RoslynML, XElement>)>
