namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
         /// <summary>
         /// Provides the element revision pair(s) to test in <see cref="XmlElementEndTagServiceProvider_RoslynMLDeltaExpander_OK"/>.
         /// </summary>
    	 ...
    	 /// <summary>
         /// Tests expansion logic for <see cref="XmlElementEndTagSyntax"/>.
         /// </summary>
    	 [TestMethod]
         public void XmlElementEndTagServiceProvider_RoslynMLDeltaExpander_OK()
         {
    		var converter = new CDF.CSharp.RoslynML.RoslynML();
    		var selector = new CDF.CSharp.RoslynML.RoslynMLPruneSelector();
    		int id = 0; 
    
    	    IEnumerable<(XmlElementEndTagSyntax, XmlElementEndTagSyntax)> nodeRevisionPairs = null;
    	    string oExpectedLabel = null, mExpectedLabel = null;
    	    XmlElementEndTagServiceProvider_RoslynMLDeltaExpander_DataToTest(ref nodeRevisionPairs);
    		foreach(((XmlElementEndTagSyntax Original, XmlElementEndTagSyntax Modified) nodeRevisionPair, Action<RoslynML, XElement> defoliate) in nodeRevisionPairs
    			.SelectMany(n => new List<((XmlElementEndTagSyntax, XmlElementEndTagSyntax), Action<RoslynML, XElement>)>
    				{ (n, (r, n1) => { }), (n, (r, n1) => r.Defoliate(n1)) }))
    		{
    			...
    			relevantDescendants = oFullElement.Elements().Single(e => e.Attribute("part").Value == "Name")
    			...
    			var oLessThanSlashTokenLabel = Enum.GetName(typeof(SyntaxKind), nodeRevisionPair.Original.LessThanSlashToken.Kind());
    			var mLessThanSlashTokenLabel = Enum.GetName(typeof(SyntaxKind), nodeRevisionPair.Modified.LessThanSlashToken.Kind());
    			...
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Label() == oLessThanSlashTokenLabel).GtID())
    				.Attribute("mId").Value == 
    				mFullElement.Elements().Single(e => e.Label() == mLessThanSlashTokenLabel).GtID());
    
    			var oGreaterThanTokenLabel = Enum.GetName(typeof(SyntaxKind), nodeRevisionPair.Original.GreaterThanToken.Kind());
    			var mGreaterThanTokenLabel = Enum.GetName(typeof(SyntaxKind), nodeRevisionPair.Modified.GreaterThanToken.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == oGreaterThanTokenLabel)
    				.Attribute("mLb").Value == mGreaterThanTokenLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Label() == oGreaterThanTokenLabel).GtID())
    				.Attribute("mId").Value == 
    				mFullElement.Elements().Single(e => e.Label() == mGreaterThanTokenLabel).GtID());
    			...
    			Assert.IsTrue(expander.FullDelta.Actions
    					.Single(a => a.Name.LocalName == "Insert" && a.Attribute("eLb").Value == mGreaterThanTokenLabel)
    					.Attribute("pLb").Value == mExpectedLabel);
    			Assert.IsTrue(expander.FullDelta.Actions
    				.Single(a => a.Name.LocalName == "Insert" && a.Attribute("eId").Value == 
    					mFullElement.Elements().Single(e => e.Label() == mGreaterThanTokenLabel).GtID())
    			...
    			if(mFullElement1.Elements().Any(e => e.Label() == mLessThanSlashTokenLabel))
    				mFullElement1.Elements().Single(e => e.Label() == mLessThanSlashTokenLabel).Value = "v0";
    			
    			if(mFullElement1.Elements().Any(e => e.Label() == mGreaterThanTokenLabel))
    				mFullElement1.Elements().Single(e => e.Label() == mGreaterThanTokenLabel).Value = "v1";
    			...
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				...
    				mFullElement.Elements().Single(e => e.Label() == mGreaterThanTokenLabel).GtID());
    			Assert.IsTrue(expander.FullDelta.Actions
    					.Single(a => a.Name.LocalName == "Update" && a.Attribute("eLb").Value == oGreaterThanTokenLabel)
