namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void XmlElementEndTagServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<XmlElementEndTagSyntax> nodes);
         ...
         public void XmlElementEndTagServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<XmlElementEndTagSyntax> nodes = null;
    	    string expectedLabel = null;
    	    XmlElementEndTagServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var LessThanSlashTokenLabel = Enum.GetName(typeof(SyntaxKind), node.LessThanSlashToken.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == LessThanSlashTokenLabel).Attribute("mLb").Value == LessThanSlashTokenLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == LessThanSlashTokenLabel).GtID())
    				.Attribute("mId").Value == mFullElement.GtID());
    
    			var GreaterThanTokenLabel = Enum.GetName(typeof(SyntaxKind), node.GreaterThanToken.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == GreaterThanTokenLabel).Attribute("mLb").Value == GreaterThanTokenLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == GreaterThanTokenLabel).GtID())
    				.Attribute("mId").Value == mFullElement.GtID());
