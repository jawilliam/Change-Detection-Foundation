namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void IndexerMemberCrefServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<IndexerMemberCrefSyntax> nodes);
         ...
         public void IndexerMemberCrefServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<IndexerMemberCrefSyntax> nodes = null;
    	    string expectedLabel = null;
    	    IndexerMemberCrefServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var ThisKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.ThisKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == ThisKeywordLabel).Attribute("mLb").Value == ThisKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == ThisKeywordLabel).GtID())
