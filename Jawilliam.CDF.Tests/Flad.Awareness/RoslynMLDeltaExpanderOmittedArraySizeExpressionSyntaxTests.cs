namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void OmittedArraySizeExpressionServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<OmittedArraySizeExpressionSyntax> nodes);
         ...
         public void OmittedArraySizeExpressionServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<OmittedArraySizeExpressionSyntax> nodes = null;
    	    string expectedLabel = null;
    	    OmittedArraySizeExpressionServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var OmittedArraySizeExpressionTokenLabel = Enum.GetName(typeof(SyntaxKind), node.OmittedArraySizeExpressionToken.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == OmittedArraySizeExpressionTokenLabel).Attribute("mLb").Value == OmittedArraySizeExpressionTokenLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == OmittedArraySizeExpressionTokenLabel).GtID())
