namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void OrderingServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<OrderingSyntax> nodes);
         ...
         public void OrderingServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<OrderingSyntax> nodes = null;
    	    string expectedLabel = null;
    	    OrderingServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var existingProperties = 1;
    			existingProperties = oFullElement.Elements().Any(e => e.Name.LocalName == "AscendingOrDescendingKeyword") ? existingProperties : (existingProperties - 1);
    			...
    			var AscendingOrDescendingKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.AscendingOrDescendingKeyword.Kind());
    			if(oFullElement.Elements().Any(e => e.Name.LocalName == "AscendingOrDescendingKeyword"))
    			{
    				Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == AscendingOrDescendingKeywordLabel).Attribute("mLb").Value == AscendingOrDescendingKeywordLabel);
    				Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    					oFullElement.Elements().Single(e => e.Attribute("oLb").Value == AscendingOrDescendingKeywordLabel).GtID())
    					.Attribute("mId").Value == mFullElement.GtID());
    			}
    			else
    			{
    				Assert.IsFalse(expander.FullDelta.Matches.Any(m => m.Attribute("oLb").Value == AscendingOrDescendingKeywordLabel);
    			}
