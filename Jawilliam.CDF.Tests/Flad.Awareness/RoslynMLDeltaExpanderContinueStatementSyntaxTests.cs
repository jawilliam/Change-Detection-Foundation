namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void ContinueStatementServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<ContinueStatementSyntax> nodes);
         ...
         public void ContinueStatementServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<ContinueStatementSyntax> nodes = null;
    	    string expectedLabel = null;
    	    ContinueStatementServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var ContinueKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.ContinueKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == ContinueKeywordLabel).Attribute("mLb").Value == ContinueKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == ContinueKeywordLabel).GtID())
