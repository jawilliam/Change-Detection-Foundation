namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void CompilationUnitServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<CompilationUnitSyntax> nodes);
         ...
         public void CompilationUnitServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<CompilationUnitSyntax> nodes = null;
    	    string expectedLabel = null;
    	    CompilationUnitServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var EndOfFileTokenLabel = Enum.GetName(typeof(SyntaxKind), node.EndOfFileToken.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == EndOfFileTokenLabel).Attribute("mLb").Value == EndOfFileTokenLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == EndOfFileTokenLabel).GtID())
