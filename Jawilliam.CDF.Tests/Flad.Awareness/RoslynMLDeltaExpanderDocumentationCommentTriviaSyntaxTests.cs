namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void DocumentationCommentTriviaServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<DocumentationCommentTriviaSyntax> nodes);
         ...
         public void DocumentationCommentTriviaServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<DocumentationCommentTriviaSyntax> nodes = null;
    	    string expectedLabel = null;
    	    DocumentationCommentTriviaServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var EndOfCommentLabel = Enum.GetName(typeof(SyntaxKind), node.EndOfComment.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == EndOfCommentLabel).Attribute("mLb").Value == EndOfCommentLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == EndOfCommentLabel).GtID())
