namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void UndefDirectiveTriviaServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<UndefDirectiveTriviaSyntax> nodes);
         ...
         public void UndefDirectiveTriviaServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<UndefDirectiveTriviaSyntax> nodes = null;
    	    string expectedLabel = null;
    	    UndefDirectiveTriviaServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var UndefKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.UndefKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == UndefKeywordLabel).Attribute("mLb").Value == UndefKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == UndefKeywordLabel).GtID())
    				.Attribute("mId").Value == mFullElement.GtID());
