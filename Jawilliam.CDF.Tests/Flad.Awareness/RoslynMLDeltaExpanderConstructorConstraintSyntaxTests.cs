namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
         /// <summary>
         /// Provides the element revision pair(s) to test in <see cref="ConstructorConstraintServiceProvider_RoslynMLDeltaExpander_OK"/>.
         /// </summary>
         /// <param name="nodeRevisionPairs"> the element revision pair(s) to test</param>
    	 partial void ConstructorConstraintServiceProvider_RoslynMLDeltaExpander_DataToTest(ref IEnumerable<(ConstructorConstraintSyntax, ConstructorConstraintSyntax)> nodeRevisionPairs);
    
    	 /// <summary>
         /// Tests expansion logic for <see cref="ConstructorConstraintSyntax"/>.
         /// </summary>
    	 [TestMethod]
         public void ConstructorConstraintServiceProvider_RoslynMLDeltaExpander_OK()
         {
    		var converter = new CDF.CSharp.RoslynML.RoslynML();
    		var selector = new CDF.CSharp.RoslynML.RoslynMLPruneSelector();
    		int id = 0; 
    
    	    IEnumerable<(ConstructorConstraintSyntax, ConstructorConstraintSyntax)> nodeRevisionPairs = null;
    	    string oExpectedLabel = null, mExpectedLabel = null;
    	    ConstructorConstraintServiceProvider_RoslynMLDeltaExpander_DataToTest(ref nodeRevisionPairs);
    		foreach(((ConstructorConstraintSyntax Original, ConstructorConstraintSyntax Modified) nodeRevisionPair, Action<RoslynML, XElement> defoliate) in nodeRevisionPairs
    			.SelectMany(n => new List<((ConstructorConstraintSyntax, ConstructorConstraintSyntax), Action<RoslynML, XElement>)>
