namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void IsPatternExpressionServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<IsPatternExpressionSyntax> nodes);
         ...
         public void IsPatternExpressionServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<IsPatternExpressionSyntax> nodes = null;
    	    string expectedLabel = null;
    	    IsPatternExpressionServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var IsKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.IsKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == IsKeywordLabel).Attribute("mLb").Value == IsKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == IsKeywordLabel).GtID())
