namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void XmlCDataSectionServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<XmlCDataSectionSyntax> nodes);
         ...
         public void XmlCDataSectionServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<XmlCDataSectionSyntax> nodes = null;
    	    string expectedLabel = null;
    	    XmlCDataSectionServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var StartCDataTokenLabel = Enum.GetName(typeof(SyntaxKind), node.StartCDataToken.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == StartCDataTokenLabel).Attribute("mLb").Value == StartCDataTokenLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == StartCDataTokenLabel).GtID())
    				.Attribute("mId").Value == mFullElement.GtID());
    
    			var EndCDataTokenLabel = Enum.GetName(typeof(SyntaxKind), node.EndCDataToken.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == EndCDataTokenLabel).Attribute("mLb").Value == EndCDataTokenLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == EndCDataTokenLabel).GtID())
    				.Attribute("mId").Value == mFullElement.GtID());
