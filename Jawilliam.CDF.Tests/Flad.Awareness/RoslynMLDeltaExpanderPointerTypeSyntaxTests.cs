namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void PointerTypeServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<PointerTypeSyntax> nodes);
         ...
         public void PointerTypeServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<PointerTypeSyntax> nodes = null;
    	    string expectedLabel = null;
    	    PointerTypeServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var AsteriskTokenLabel = Enum.GetName(typeof(SyntaxKind), node.AsteriskToken.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == AsteriskTokenLabel).Attribute("mLb").Value == AsteriskTokenLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == AsteriskTokenLabel).GtID())
