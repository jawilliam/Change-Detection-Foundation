namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void ExternAliasDirectiveServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<ExternAliasDirectiveSyntax> nodes);
         ...
         public void ExternAliasDirectiveServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<ExternAliasDirectiveSyntax> nodes = null;
    	    string expectedLabel = null;
    	    ExternAliasDirectiveServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var ExternKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.ExternKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == ExternKeywordLabel).Attribute("mLb").Value == ExternKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == ExternKeywordLabel).GtID())
    				.Attribute("mId").Value == mFullElement.GtID());
    
    			var AliasKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.AliasKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == AliasKeywordLabel).Attribute("mLb").Value == AliasKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == AliasKeywordLabel).GtID())
    				.Attribute("mId").Value == mFullElement.GtID());
