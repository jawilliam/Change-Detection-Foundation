namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
         /// <summary>
         /// Provides the element revision pair(s) to test in <see cref="ClassOrStructConstraintServiceProvider_RoslynMLDeltaExpander_OK"/>.
         /// </summary>
    	 ...
    	 /// <summary>
         /// Tests expansion logic for <see cref="ClassOrStructConstraintSyntax"/>.
         /// </summary>
    	 [TestMethod]
         public void ClassOrStructConstraintServiceProvider_RoslynMLDeltaExpander_OK()
         {
    		var converter = new CDF.CSharp.RoslynML.RoslynML();
    		var selector = new CDF.CSharp.RoslynML.RoslynMLPruneSelector();
    		int id = 0; 
    
    	    IEnumerable<(ClassOrStructConstraintSyntax, ClassOrStructConstraintSyntax)> nodeRevisionPairs = null;
    	    string oExpectedLabel = null, mExpectedLabel = null;
    	    ClassOrStructConstraintServiceProvider_RoslynMLDeltaExpander_DataToTest(ref nodeRevisionPairs);
    		foreach(((ClassOrStructConstraintSyntax Original, ClassOrStructConstraintSyntax Modified) nodeRevisionPair, Action<RoslynML, XElement> defoliate) in nodeRevisionPairs
    			.SelectMany(n => new List<((ClassOrStructConstraintSyntax, ClassOrStructConstraintSyntax), Action<RoslynML, XElement>)>
    				{ (n, (r, n1) => { }), (n, (r, n1) => r.Defoliate(n1)) }))
    		{
    			...
    			var oClassOrStructKeywordLabel = Enum.GetName(typeof(SyntaxKind), nodeRevisionPair.Original.ClassOrStructKeyword.Kind());
    			var mClassOrStructKeywordLabel = Enum.GetName(typeof(SyntaxKind), nodeRevisionPair.Modified.ClassOrStructKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == oClassOrStructKeywordLabel)
    				.Attribute("mLb").Value == mClassOrStructKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Label() == oClassOrStructKeywordLabel).GtID())
    				.Attribute("mId").Value == 
    				mFullElement.Elements().Single(e => e.Label() == mClassOrStructKeywordLabel).GtID());
    			...
    			Assert.IsTrue(expander.FullDelta.Actions
    					.Single(a => a.Name.LocalName == "Insert" && a.Attribute("eLb").Value == mClassOrStructKeywordLabel)
    					.Attribute("pLb").Value == mExpectedLabel);
    			Assert.IsTrue(expander.FullDelta.Actions
    			...
    			Assert.IsTrue(expander.FullDelta.Actions
    				.Single(a => a.Name.LocalName == "Delete" && a.Attribute("eId").Value == 
    					oFullElement.Elements().Single(e => e.Label() == oClassOrStructKeywordLabel).GtID())
    				.Attribute("eLb").Value == oClassOrStructKeywordLabel);
    			...
    			if(mFullElement1.Elements().Any(e => e.Label() == mClassOrStructKeywordLabel))
    				mFullElement1.Elements().Single(e => e.Label() == mClassOrStructKeywordLabel).Value = "v0";
    			...
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				...
    				mFullElement.Elements().Single(e => e.Label() == mClassOrStructKeywordLabel).GtID());
    			Assert.IsTrue(expander.FullDelta.Actions
    					.Single(a => a.Name.LocalName == "Update" && a.Attribute("eLb").Value == oClassOrStructKeywordLabel)
