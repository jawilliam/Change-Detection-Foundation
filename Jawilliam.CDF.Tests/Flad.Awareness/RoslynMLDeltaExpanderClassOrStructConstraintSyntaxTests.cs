namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void ClassOrStructConstraintServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<ClassOrStructConstraintSyntax> nodes);
         ...
         public void ClassOrStructConstraintServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<ClassOrStructConstraintSyntax> nodes = null;
    	    string expectedLabel = null;
    	    ClassOrStructConstraintServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var ClassOrStructKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.ClassOrStructKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == ClassOrStructKeywordLabel).Attribute("mLb").Value == ClassOrStructKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == ClassOrStructKeywordLabel).GtID())
