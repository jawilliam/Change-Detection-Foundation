namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void ExplicitInterfaceSpecifierServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<ExplicitInterfaceSpecifierSyntax> nodes);
         ...
         public void ExplicitInterfaceSpecifierServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<ExplicitInterfaceSpecifierSyntax> nodes = null;
    	    string expectedLabel = null;
    	    ExplicitInterfaceSpecifierServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var DotTokenLabel = Enum.GetName(typeof(SyntaxKind), node.DotToken.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == DotTokenLabel).Attribute("mLb").Value == DotTokenLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == DotTokenLabel).GtID())
