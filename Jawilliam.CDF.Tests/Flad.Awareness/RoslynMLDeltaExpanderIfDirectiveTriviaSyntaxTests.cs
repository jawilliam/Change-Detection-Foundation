namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void IfDirectiveTriviaServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<IfDirectiveTriviaSyntax> nodes);
         ...
         public void IfDirectiveTriviaServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<IfDirectiveTriviaSyntax> nodes = null;
    	    string expectedLabel = null;
    	    IfDirectiveTriviaServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var IfKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.IfKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == IfKeywordLabel).Attribute("mLb").Value == IfKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == IfKeywordLabel).GtID())
    				.Attribute("mId").Value == mFullElement.GtID());
