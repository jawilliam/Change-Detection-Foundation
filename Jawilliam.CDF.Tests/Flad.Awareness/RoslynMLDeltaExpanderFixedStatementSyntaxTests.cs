namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void FixedStatementServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<FixedStatementSyntax> nodes);
         ...
         public void FixedStatementServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<FixedStatementSyntax> nodes = null;
    	    string expectedLabel = null;
    	    FixedStatementServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var FixedKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.FixedKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == FixedKeywordLabel).Attribute("mLb").Value == FixedKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == FixedKeywordLabel).GtID())
