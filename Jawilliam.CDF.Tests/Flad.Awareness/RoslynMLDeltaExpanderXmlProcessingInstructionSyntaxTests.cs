namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void XmlProcessingInstructionServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<XmlProcessingInstructionSyntax> nodes);
         ...
         public void XmlProcessingInstructionServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<XmlProcessingInstructionSyntax> nodes = null;
    	    string expectedLabel = null;
    	    XmlProcessingInstructionServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var StartProcessingInstructionTokenLabel = Enum.GetName(typeof(SyntaxKind), node.StartProcessingInstructionToken.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == StartProcessingInstructionTokenLabel).Attribute("mLb").Value == StartProcessingInstructionTokenLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == StartProcessingInstructionTokenLabel).GtID())
    				.Attribute("mId").Value == mFullElement.GtID());
    
    			var EndProcessingInstructionTokenLabel = Enum.GetName(typeof(SyntaxKind), node.EndProcessingInstructionToken.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == EndProcessingInstructionTokenLabel).Attribute("mLb").Value == EndProcessingInstructionTokenLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == EndProcessingInstructionTokenLabel).GtID())
    				.Attribute("mId").Value == mFullElement.GtID());
