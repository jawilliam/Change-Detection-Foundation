namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
         /// <summary>
         /// Provides the element revision pair(s) to test in <see cref="XmlProcessingInstructionServiceProvider_RoslynMLDeltaExpander_OK"/>.
         /// </summary>
    	 ...
    	 /// <summary>
         /// Tests expansion logic for <see cref="XmlProcessingInstructionSyntax"/>.
         /// </summary>
    	 [TestMethod]
         public void XmlProcessingInstructionServiceProvider_RoslynMLDeltaExpander_OK()
         {
    		var converter = new CDF.CSharp.RoslynML.RoslynML();
    		var selector = new CDF.CSharp.RoslynML.RoslynMLPruneSelector();
    		int id = 0; 
    
    	    IEnumerable<(XmlProcessingInstructionSyntax, XmlProcessingInstructionSyntax)> nodeRevisionPairs = null;
    	    string oExpectedLabel = null, mExpectedLabel = null;
    	    XmlProcessingInstructionServiceProvider_RoslynMLDeltaExpander_DataToTest(ref nodeRevisionPairs);
    		foreach(((XmlProcessingInstructionSyntax Original, XmlProcessingInstructionSyntax Modified) nodeRevisionPair, Action<RoslynML, XElement> defoliate) in nodeRevisionPairs
    			.SelectMany(n => new List<((XmlProcessingInstructionSyntax, XmlProcessingInstructionSyntax), Action<RoslynML, XElement>)>
    				{ (n, (r, n1) => { }), (n, (r, n1) => r.Defoliate(n1)) }))
    		{
    			...
    			var oStartProcessingInstructionTokenLabel = Enum.GetName(typeof(SyntaxKind), nodeRevisionPair.Original.StartProcessingInstructionToken.Kind());
    			var mStartProcessingInstructionTokenLabel = Enum.GetName(typeof(SyntaxKind), nodeRevisionPair.Modified.StartProcessingInstructionToken.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == oStartProcessingInstructionTokenLabel)
    				.Attribute("mLb").Value == mStartProcessingInstructionTokenLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Label() == oStartProcessingInstructionTokenLabel).GtID())
    				.Attribute("mId").Value == 
    				mFullElement.Elements().Single(e => e.Label() == mStartProcessingInstructionTokenLabel).GtID());
    
    			var oEndProcessingInstructionTokenLabel = Enum.GetName(typeof(SyntaxKind), nodeRevisionPair.Original.EndProcessingInstructionToken.Kind());
    			var mEndProcessingInstructionTokenLabel = Enum.GetName(typeof(SyntaxKind), nodeRevisionPair.Modified.EndProcessingInstructionToken.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == oEndProcessingInstructionTokenLabel)
    				.Attribute("mLb").Value == mEndProcessingInstructionTokenLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Label() == oEndProcessingInstructionTokenLabel).GtID())
    				.Attribute("mId").Value == 
    				mFullElement.Elements().Single(e => e.Label() == mEndProcessingInstructionTokenLabel).GtID());
    			...
    			Assert.IsTrue(expander.FullDelta.Actions
    					.Single(a => a.Name.LocalName == "Insert" && a.Attribute("eLb").Value == mStartProcessingInstructionTokenLabel)
    					.Attribute("pLb").Value == mExpectedLabel);
    			Assert.IsTrue(expander.FullDelta.Actions
    			...
    			Assert.IsTrue(expander.FullDelta.Actions
    					.Single(a => a.Name.LocalName == "Insert" && a.Attribute("eLb").Value == mEndProcessingInstructionTokenLabel)
    					.Attribute("pLb").Value == mExpectedLabel);
    			Assert.IsTrue(expander.FullDelta.Actions
    				.Single(a => a.Name.LocalName == "Insert" && a.Attribute("eId").Value == 
    					mFullElement.Elements().Single(e => e.Label() == mEndProcessingInstructionTokenLabel).GtID())
    			...
    			Assert.IsTrue(expander.FullDelta.Actions
    				.Single(a => a.Name.LocalName == "Delete" && a.Attribute("eId").Value == 
    					oFullElement.Elements().Single(e => e.Label() == oStartProcessingInstructionTokenLabel).GtID())
    				.Attribute("eLb").Value == oStartProcessingInstructionTokenLabel);
    
    			Assert.IsTrue(expander.FullDelta.Actions
    				.Single(a => a.Name.LocalName == "Delete" && a.Attribute("eId").Value == 
    					oFullElement.Elements().Single(e => e.Label() == oEndProcessingInstructionTokenLabel).GtID())
    				.Attribute("eLb").Value == oEndProcessingInstructionTokenLabel);
    			...
    			if(mFullElement1.Elements().Any(e => e.Label() == mStartProcessingInstructionTokenLabel))
    				mFullElement1.Elements().Single(e => e.Label() == mStartProcessingInstructionTokenLabel).Value = "v0";
    			
    			if(mFullElement1.Elements().Any(e => e.Label() == mEndProcessingInstructionTokenLabel))
    				mFullElement1.Elements().Single(e => e.Label() == mEndProcessingInstructionTokenLabel).Value = "v1";
    			...
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				...
    				mFullElement.Elements().Single(e => e.Label() == mStartProcessingInstructionTokenLabel).GtID());
    			Assert.IsTrue(expander.FullDelta.Actions
    					.Single(a => a.Name.LocalName == "Update" && a.Attribute("eLb").Value == oStartProcessingInstructionTokenLabel)
    			...
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				...
    				mFullElement.Elements().Single(e => e.Label() == mEndProcessingInstructionTokenLabel).GtID());
    			Assert.IsTrue(expander.FullDelta.Actions
    					.Single(a => a.Name.LocalName == "Update" && a.Attribute("eLb").Value == oEndProcessingInstructionTokenLabel)
