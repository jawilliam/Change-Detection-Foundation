namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void DoStatementServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<DoStatementSyntax> nodes);
         ...
         public void DoStatementServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<DoStatementSyntax> nodes = null;
    	    string expectedLabel = null;
    	    DoStatementServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var DoKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.DoKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == DoKeywordLabel).Attribute("mLb").Value == DoKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == DoKeywordLabel).GtID())
    				.Attribute("mId").Value == mFullElement.GtID());
    
    			var WhileKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.WhileKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == WhileKeywordLabel).Attribute("mLb").Value == WhileKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == WhileKeywordLabel).GtID())
    				.Attribute("mId").Value == mFullElement.GtID());
