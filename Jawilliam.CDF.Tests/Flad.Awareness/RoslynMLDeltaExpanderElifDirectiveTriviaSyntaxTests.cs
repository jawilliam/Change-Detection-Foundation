namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void ElifDirectiveTriviaServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<ElifDirectiveTriviaSyntax> nodes);
         ...
         public void ElifDirectiveTriviaServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<ElifDirectiveTriviaSyntax> nodes = null;
    	    string expectedLabel = null;
    	    ElifDirectiveTriviaServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var ElifKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.ElifKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == ElifKeywordLabel).Attribute("mLb").Value == ElifKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == ElifKeywordLabel).GtID())
    				.Attribute("mId").Value == mFullElement.GtID());
