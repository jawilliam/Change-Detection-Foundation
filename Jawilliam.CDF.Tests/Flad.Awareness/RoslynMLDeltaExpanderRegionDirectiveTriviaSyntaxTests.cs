namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void RegionDirectiveTriviaServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<RegionDirectiveTriviaSyntax> nodes);
         ...
         public void RegionDirectiveTriviaServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<RegionDirectiveTriviaSyntax> nodes = null;
    	    string expectedLabel = null;
    	    RegionDirectiveTriviaServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var RegionKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.RegionKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == RegionKeywordLabel).Attribute("mLb").Value == RegionKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == RegionKeywordLabel).GtID())
    				.Attribute("mId").Value == mFullElement.GtID());
