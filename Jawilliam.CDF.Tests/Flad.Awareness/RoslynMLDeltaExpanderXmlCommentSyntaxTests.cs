namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void XmlCommentServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<XmlCommentSyntax> nodes);
         ...
         public void XmlCommentServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<XmlCommentSyntax> nodes = null;
    	    string expectedLabel = null;
    	    XmlCommentServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var LessThanExclamationMinusMinusTokenLabel = Enum.GetName(typeof(SyntaxKind), node.LessThanExclamationMinusMinusToken.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == LessThanExclamationMinusMinusTokenLabel).Attribute("mLb").Value == LessThanExclamationMinusMinusTokenLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == LessThanExclamationMinusMinusTokenLabel).GtID())
    				.Attribute("mId").Value == mFullElement.GtID());
    
    			var MinusMinusGreaterThanTokenLabel = Enum.GetName(typeof(SyntaxKind), node.MinusMinusGreaterThanToken.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == MinusMinusGreaterThanTokenLabel).Attribute("mLb").Value == MinusMinusGreaterThanTokenLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == MinusMinusGreaterThanTokenLabel).GtID())
    				.Attribute("mId").Value == mFullElement.GtID());
