namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void ConditionalExpressionServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<ConditionalExpressionSyntax> nodes);
         ...
         public void ConditionalExpressionServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<ConditionalExpressionSyntax> nodes = null;
    	    string expectedLabel = null;
    	    ConditionalExpressionServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var QuestionTokenLabel = Enum.GetName(typeof(SyntaxKind), node.QuestionToken.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == QuestionTokenLabel).Attribute("mLb").Value == QuestionTokenLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == QuestionTokenLabel).GtID())
