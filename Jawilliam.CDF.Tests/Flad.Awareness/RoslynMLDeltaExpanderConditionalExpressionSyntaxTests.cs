namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    partial class RoslynMLDeltaExpanderTests
    {
         ...
         public void ConditionalExpressionServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<(ConditionalExpressionSyntax, ConditionalExpressionSyntax)> nodeRevisionPairs = null;
    	    string oExpectedLabel = null, mExpectedLabel = null;
    	    ConditionalExpressionServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodeRevisionPairs);
    		foreach(((ConditionalExpressionSyntax Original, ConditionalExpressionSyntax Modified) nodeRevisionPair, Action<RoslynML, XElement> defoliate) in nodeRevisionPairs
    			.SelectMany(n => new List<((ConditionalExpressionSyntax, ConditionalExpressionSyntax), Action<RoslynML, XElement>)>
    				{ (n, (r, n1) => { }), (n, (r, n1) => r.Defoliate(n1)) }))
    		{
    			...
    			var oQuestionTokenLabel = Enum.GetName(typeof(SyntaxKind), nodeRevisionPair.Original.QuestionToken.Kind());
    			var mQuestionTokenLabel = Enum.GetName(typeof(SyntaxKind), nodeRevisionPair.Modified.QuestionToken.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == oQuestionTokenLabel)
    				.Attribute("mLb").Value == mQuestionTokenLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Label() == oQuestionTokenLabel).GtID())
    				.Attribute("mId").Value == 
    				mFullElement.Elements().Single(e => e.Label() == mQuestionTokenLabel).GtID());
    			...
    			Assert.IsTrue(expander.FullDelta.Actions
    					.Single(a => a.Name.LocalName == "Insert" && a.Attribute("eLb").Value == mQuestionTokenLabel)
    					.Attribute("pLb").Value == oExpectedLabel);
    			Assert.IsTrue(expander.FullDelta.Actions
    			...
    			Assert.IsTrue(expander.FullDelta.Actions
    				.Single(a => a.Name.LocalName == "Delete" && a.Attribute("eId").Value == 
    					oFullElement.Elements().Single(e => e.Label() == oQuestionTokenLabel).GtID())
    				.Attribute("eLb").Value == oQuestionTokenLabel);
    			...
    			if(mFullElement1.Elements().Any(e => e.Label() == mQuestionTokenLabel))
    				mFullElement1.Elements().Single(e => e.Label() == mQuestionTokenLabel).Value = "v0";
    			...
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				...
    				mFullElement.Elements().Single(e => e.Label() == mQuestionTokenLabel).GtID());
    			Assert.IsTrue(expander.FullDelta.Actions
    					.Single(a => a.Name.LocalName == "Update" && a.Attribute("eLb").Value == oQuestionTokenLabel)
