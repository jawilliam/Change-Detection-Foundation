namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void CatchFilterClauseServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<CatchFilterClauseSyntax> nodes);
         ...
         public void CatchFilterClauseServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<CatchFilterClauseSyntax> nodes = null;
    	    string expectedLabel = null;
    	    CatchFilterClauseServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var WhenKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.WhenKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == WhenKeywordLabel).Attribute("mLb").Value == WhenKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == WhenKeywordLabel).GtID())
