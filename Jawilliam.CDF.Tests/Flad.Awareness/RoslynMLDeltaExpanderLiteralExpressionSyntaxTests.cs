namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void LiteralExpressionServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<LiteralExpressionSyntax> nodes);
         ...
         public void LiteralExpressionServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<LiteralExpressionSyntax> nodes = null;
    	    string expectedLabel = null;
    	    LiteralExpressionServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var TokenLabel = Enum.GetName(typeof(SyntaxKind), node.Token.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == TokenLabel).Attribute("mLb").Value == TokenLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == TokenLabel).GtID())
