namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void CatchClauseServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<CatchClauseSyntax> nodes);
         ...
         public void CatchClauseServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<CatchClauseSyntax> nodes = null;
    	    string expectedLabel = null;
    	    CatchClauseServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var CatchKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.CatchKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == CatchKeywordLabel).Attribute("mLb").Value == CatchKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == CatchKeywordLabel).GtID())
