namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void ConditionalAccessExpressionServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<ConditionalAccessExpressionSyntax> nodes);
         ...
         public void ConditionalAccessExpressionServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<ConditionalAccessExpressionSyntax> nodes = null;
    	    string expectedLabel = null;
    	    ConditionalAccessExpressionServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var OperatorTokenLabel = Enum.GetName(typeof(SyntaxKind), node.OperatorToken.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == OperatorTokenLabel).Attribute("mLb").Value == OperatorTokenLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == OperatorTokenLabel).GtID())
