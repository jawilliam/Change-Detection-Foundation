namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void ConstructorInitializerServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<ConstructorInitializerSyntax> nodes);
         ...
         public void ConstructorInitializerServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<ConstructorInitializerSyntax> nodes = null;
    	    string expectedLabel = null;
    	    ConstructorInitializerServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var ThisOrBaseKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.ThisOrBaseKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == ThisOrBaseKeywordLabel).Attribute("mLb").Value == ThisOrBaseKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == ThisOrBaseKeywordLabel).GtID())
    				.Attribute("mId").Value == mFullElement.GtID());
