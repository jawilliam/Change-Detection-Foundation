namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void UnsafeStatementServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<UnsafeStatementSyntax> nodes);
         ...
         public void UnsafeStatementServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<UnsafeStatementSyntax> nodes = null;
    	    string expectedLabel = null;
    	    UnsafeStatementServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var UnsafeKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.UnsafeKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == UnsafeKeywordLabel).Attribute("mLb").Value == UnsafeKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == UnsafeKeywordLabel).GtID())
