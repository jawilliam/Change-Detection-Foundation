namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void GotoStatementServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<GotoStatementSyntax> nodes);
         ...
         public void GotoStatementServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<GotoStatementSyntax> nodes = null;
    	    string expectedLabel = null;
    	    GotoStatementServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			existingProperties = oFullElement.Elements().Any(e => e.Name.LocalName == "CaseOrDefaultKeyword") ? existingProperties : (existingProperties - 1);
    			...
    			var GotoKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.GotoKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == GotoKeywordLabel).Attribute("mLb").Value == GotoKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == GotoKeywordLabel).GtID())
    				.Attribute("mId").Value == mFullElement.GtID());
    
    			var CaseOrDefaultKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.CaseOrDefaultKeyword.Kind());
    			if(oFullElement.Elements().Any(e => e.Name.LocalName == "CaseOrDefaultKeyword"))
    			{
    				Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == CaseOrDefaultKeywordLabel).Attribute("mLb").Value == CaseOrDefaultKeywordLabel);
    				Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    					oFullElement.Elements().Single(e => e.Attribute("oLb").Value == CaseOrDefaultKeywordLabel).GtID())
    					.Attribute("mId").Value == mFullElement.GtID());
    			}
    			else
    			{
    				Assert.IsFalse(expander.FullDelta.Matches.Any(m => m.Attribute("oLb").Value == CaseOrDefaultKeywordLabel);
    			}
