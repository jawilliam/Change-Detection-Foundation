namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void ElseDirectiveTriviaServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<ElseDirectiveTriviaSyntax> nodes);
         ...
         public void ElseDirectiveTriviaServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<ElseDirectiveTriviaSyntax> nodes = null;
    	    string expectedLabel = null;
    	    ElseDirectiveTriviaServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var ElseKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.ElseKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == ElseKeywordLabel).Attribute("mLb").Value == ElseKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == ElseKeywordLabel).GtID())
    				.Attribute("mId").Value == mFullElement.GtID());
