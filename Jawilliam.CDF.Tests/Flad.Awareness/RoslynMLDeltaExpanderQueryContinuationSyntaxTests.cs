namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void QueryContinuationServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<QueryContinuationSyntax> nodes);
         ...
         public void QueryContinuationServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<QueryContinuationSyntax> nodes = null;
    	    string expectedLabel = null;
    	    QueryContinuationServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var IntoKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.IntoKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == IntoKeywordLabel).Attribute("mLb").Value == IntoKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == IntoKeywordLabel).GtID())
