namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void UsingDirectiveServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<UsingDirectiveSyntax> nodes);
         ...
         public void UsingDirectiveServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<UsingDirectiveSyntax> nodes = null;
    	    string expectedLabel = null;
    	    UsingDirectiveServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			existingProperties = oFullElement.Elements().Any(e => e.Name.LocalName == "StaticKeyword") ? existingProperties : (existingProperties - 1);
    			...
    			var StaticKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.StaticKeyword.Kind());
    			if(oFullElement.Elements().Any(e => e.Name.LocalName == "StaticKeyword"))
    			{
    				Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == StaticKeywordLabel).Attribute("mLb").Value == StaticKeywordLabel);
    				Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    					oFullElement.Elements().Single(e => e.Attribute("oLb").Value == StaticKeywordLabel).GtID())
    					.Attribute("mId").Value == mFullElement.GtID());
    			}
    			else
    			{
    				Assert.IsFalse(expander.FullDelta.Matches.Any(m => m.Attribute("oLb").Value == StaticKeywordLabel);
    			}
