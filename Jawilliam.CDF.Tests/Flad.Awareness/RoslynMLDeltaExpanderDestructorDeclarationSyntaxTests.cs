namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void DestructorDeclarationServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<DestructorDeclarationSyntax> nodes);
         ...
         public void DestructorDeclarationServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<DestructorDeclarationSyntax> nodes = null;
    	    string expectedLabel = null;
    	    DestructorDeclarationServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var TildeTokenLabel = Enum.GetName(typeof(SyntaxKind), node.TildeToken.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == TildeTokenLabel).Attribute("mLb").Value == TildeTokenLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == TildeTokenLabel).GtID())
