namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void ShebangDirectiveTriviaServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<ShebangDirectiveTriviaSyntax> nodes);
         ...
         public void ShebangDirectiveTriviaServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<ShebangDirectiveTriviaSyntax> nodes = null;
    	    string expectedLabel = null;
    	    ShebangDirectiveTriviaServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var ExclamationTokenLabel = Enum.GetName(typeof(SyntaxKind), node.ExclamationToken.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == ExclamationTokenLabel).Attribute("mLb").Value == ExclamationTokenLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == ExclamationTokenLabel).GtID())
    				.Attribute("mId").Value == mFullElement.GtID());
