namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void OrderByClauseServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<OrderByClauseSyntax> nodes);
         ...
         public void OrderByClauseServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<OrderByClauseSyntax> nodes = null;
    	    string expectedLabel = null;
    	    OrderByClauseServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var OrderByKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.OrderByKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == OrderByKeywordLabel).Attribute("mLb").Value == OrderByKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == OrderByKeywordLabel).GtID())
