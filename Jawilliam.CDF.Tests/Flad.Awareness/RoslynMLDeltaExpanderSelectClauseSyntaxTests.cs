namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void SelectClauseServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<SelectClauseSyntax> nodes);
         ...
         public void SelectClauseServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<SelectClauseSyntax> nodes = null;
    	    string expectedLabel = null;
    	    SelectClauseServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var SelectKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.SelectKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == SelectKeywordLabel).Attribute("mLb").Value == SelectKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == SelectKeywordLabel).GtID())
