namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void XmlCrefAttributeServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<XmlCrefAttributeSyntax> nodes);
         ...
         public void XmlCrefAttributeServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<XmlCrefAttributeSyntax> nodes = null;
    	    string expectedLabel = null;
    	    XmlCrefAttributeServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var NameLabel = Enum.GetName(typeof(SyntaxKind), node.Name.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == NameLabel).Attribute("mLb").Value == NameLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == NameLabel).GtID())
    				.Attribute("mId").Value == mFullElement.GtID());
    
    			var EqualsTokenLabel = Enum.GetName(typeof(SyntaxKind), node.EqualsToken.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == EqualsTokenLabel).Attribute("mLb").Value == EqualsTokenLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == EqualsTokenLabel).GtID())
    				.Attribute("mId").Value == mFullElement.GtID());
    
    			var StartQuoteTokenLabel = Enum.GetName(typeof(SyntaxKind), node.StartQuoteToken.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == StartQuoteTokenLabel).Attribute("mLb").Value == StartQuoteTokenLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == StartQuoteTokenLabel).GtID())
    				.Attribute("mId").Value == mFullElement.GtID());
    
    			var EndQuoteTokenLabel = Enum.GetName(typeof(SyntaxKind), node.EndQuoteToken.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == EndQuoteTokenLabel).Attribute("mLb").Value == EndQuoteTokenLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == EndQuoteTokenLabel).GtID())
    				.Attribute("mId").Value == mFullElement.GtID());
