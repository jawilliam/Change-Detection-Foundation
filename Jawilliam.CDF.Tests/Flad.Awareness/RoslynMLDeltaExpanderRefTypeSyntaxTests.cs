namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void RefTypeServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<RefTypeSyntax> nodes);
         ...
         public void RefTypeServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<RefTypeSyntax> nodes = null;
    	    string expectedLabel = null;
    	    RefTypeServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			existingProperties = oFullElement.Elements().Any(e => e.Name.LocalName == "ReadOnlyKeyword") ? existingProperties : (existingProperties - 1);
    			...
    			var RefKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.RefKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == RefKeywordLabel).Attribute("mLb").Value == RefKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == RefKeywordLabel).GtID())
    				.Attribute("mId").Value == mFullElement.GtID());
    
    			var ReadOnlyKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.ReadOnlyKeyword.Kind());
    			if(oFullElement.Elements().Any(e => e.Name.LocalName == "ReadOnlyKeyword"))
    			{
    				Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == ReadOnlyKeywordLabel).Attribute("mLb").Value == ReadOnlyKeywordLabel);
    				Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    					oFullElement.Elements().Single(e => e.Attribute("oLb").Value == ReadOnlyKeywordLabel).GtID())
    					.Attribute("mId").Value == mFullElement.GtID());
    			}
    			else
    			{
    				Assert.IsFalse(expander.FullDelta.Matches.Any(m => m.Attribute("oLb").Value == ReadOnlyKeywordLabel);
    			}
