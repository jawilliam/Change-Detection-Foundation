namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void InterpolationAlignmentClauseServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<InterpolationAlignmentClauseSyntax> nodes);
         ...
         public void InterpolationAlignmentClauseServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<InterpolationAlignmentClauseSyntax> nodes = null;
    	    string expectedLabel = null;
    	    InterpolationAlignmentClauseServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var CommaTokenLabel = Enum.GetName(typeof(SyntaxKind), node.CommaToken.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == CommaTokenLabel).Attribute("mLb").Value == CommaTokenLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == CommaTokenLabel).GtID())
