namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void InterpolationFormatClauseServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<InterpolationFormatClauseSyntax> nodes);
         ...
         public void InterpolationFormatClauseServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<InterpolationFormatClauseSyntax> nodes = null;
    	    string expectedLabel = null;
    	    InterpolationFormatClauseServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var FormatStringTokenLabel = Enum.GetName(typeof(SyntaxKind), node.FormatStringToken.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == FormatStringTokenLabel).Attribute("mLb").Value == FormatStringTokenLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == FormatStringTokenLabel).GtID())
    				.Attribute("mId").Value == mFullElement.GtID());
