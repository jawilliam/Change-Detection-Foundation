    ...
    partial class RoslynMLDeltaExpanderTests
    {
         ...
         public void XmlElementServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    				...
    				{ (n, (r, n1) => { }), (n, (r, n1) => r.Defoliate(n1)) }))
    		{
    			...
    			if(mFullElement1.Elements().Any(e => e.Label() == mStartTagLabel))
    				mFullElement1.Elements().Single(e => e.Label() == mStartTagLabel).Value = "v0";
    			
    			if(mFullElement1.Elements().Any(e => e.Label() == mEndTagLabel))
    				mFullElement1.Elements().Single(e => e.Label() == mEndTagLabel).Value = "v1";
    			...
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				...
    				mFullElement.Elements().Single(e => e.Label() == mStartTagLabel).GtID());
    			Assert.IsTrue(expander.FullDelta.Actions
    					.Single(a => a.Name.LocalName == "Update" && a.Attribute("eLb").Value == oStartTagLabel)
    			...
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				...
    				mFullElement.Elements().Single(e => e.Label() == mEndTagLabel).GtID());
    			Assert.IsTrue(expander.FullDelta.Actions
    					.Single(a => a.Name.LocalName == "Update" && a.Attribute("eLb").Value == oEndTagLabel)
