namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
         /// <summary>
         /// Provides the element revision pair(s) to test in <see cref="ParenthesizedExpressionServiceProvider_RoslynMLDeltaExpander_OK"/>.
         /// </summary>
    	 ...
    	 /// <summary>
         /// Tests expansion logic for <see cref="ParenthesizedExpressionSyntax"/>.
         /// </summary>
    	 [TestMethod]
         public void ParenthesizedExpressionServiceProvider_RoslynMLDeltaExpander_OK()
         {
    		var converter = new CDF.CSharp.RoslynML.RoslynML();
    		var selector = new CDF.CSharp.RoslynML.RoslynMLPruneSelector();
    		int id = 0; 
    
    	    IEnumerable<(ParenthesizedExpressionSyntax, ParenthesizedExpressionSyntax)> nodeRevisionPairs = null;
    	    string oExpectedLabel = null, mExpectedLabel = null;
    	    ParenthesizedExpressionServiceProvider_RoslynMLDeltaExpander_DataToTest(ref nodeRevisionPairs);
    		foreach(((ParenthesizedExpressionSyntax Original, ParenthesizedExpressionSyntax Modified) nodeRevisionPair, Action<RoslynML, XElement> defoliate) in nodeRevisionPairs
    			.SelectMany(n => new List<((ParenthesizedExpressionSyntax, ParenthesizedExpressionSyntax), Action<RoslynML, XElement>)>
