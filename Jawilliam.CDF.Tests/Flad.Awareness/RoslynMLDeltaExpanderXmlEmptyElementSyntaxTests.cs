namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
         /// <summary>
         /// Provides the element revision pair(s) to test in <see cref="XmlEmptyElementServiceProvider_RoslynMLDeltaExpander_OK"/>.
         /// </summary>
    	 ...
    	 /// <summary>
         /// Tests expansion logic for <see cref="XmlEmptyElementSyntax"/>.
         /// </summary>
    	 [TestMethod]
         public void XmlEmptyElementServiceProvider_RoslynMLDeltaExpander_OK()
         {
    		var converter = new CDF.CSharp.RoslynML.RoslynML();
    		var selector = new CDF.CSharp.RoslynML.RoslynMLPruneSelector();
    		int id = 0; 
    
    	    IEnumerable<(XmlEmptyElementSyntax, XmlEmptyElementSyntax)> nodeRevisionPairs = null;
    	    string oExpectedLabel = null, mExpectedLabel = null;
    	    XmlEmptyElementServiceProvider_RoslynMLDeltaExpander_DataToTest(ref nodeRevisionPairs);
    		foreach(((XmlEmptyElementSyntax Original, XmlEmptyElementSyntax Modified) nodeRevisionPair, Action<RoslynML, XElement> defoliate) in nodeRevisionPairs
    			.SelectMany(n => new List<((XmlEmptyElementSyntax, XmlEmptyElementSyntax), Action<RoslynML, XElement>)>
    				{ (n, (r, n1) => { }), (n, (r, n1) => r.Defoliate(n1)) }))
    		{
    			...
    			var oLessThanTokenLabel = Enum.GetName(typeof(SyntaxKind), nodeRevisionPair.Original.LessThanToken.Kind());
    			var mLessThanTokenLabel = Enum.GetName(typeof(SyntaxKind), nodeRevisionPair.Modified.LessThanToken.Kind());
    			...
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Label() == oLessThanTokenLabel).GtID())
    				.Attribute("mId").Value == 
    				mFullElement.Elements().Single(e => e.Label() == mLessThanTokenLabel).GtID());
    
    			var oSlashGreaterThanTokenLabel = Enum.GetName(typeof(SyntaxKind), nodeRevisionPair.Original.SlashGreaterThanToken.Kind());
    			var mSlashGreaterThanTokenLabel = Enum.GetName(typeof(SyntaxKind), nodeRevisionPair.Modified.SlashGreaterThanToken.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == oSlashGreaterThanTokenLabel)
    				.Attribute("mLb").Value == mSlashGreaterThanTokenLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Label() == oSlashGreaterThanTokenLabel).GtID())
    				.Attribute("mId").Value == 
    				mFullElement.Elements().Single(e => e.Label() == mSlashGreaterThanTokenLabel).GtID());
    			...
    			Assert.IsTrue(expander.FullDelta.Actions
    					.Single(a => a.Name.LocalName == "Insert" && a.Attribute("eLb").Value == mSlashGreaterThanTokenLabel)
    					.Attribute("pLb").Value == mExpectedLabel);
    			Assert.IsTrue(expander.FullDelta.Actions
    				.Single(a => a.Name.LocalName == "Insert" && a.Attribute("eId").Value == 
    					mFullElement.Elements().Single(e => e.Label() == mSlashGreaterThanTokenLabel).GtID())
    			...
    			Assert.IsTrue(expander.FullDelta.Actions
    				.Single(a => a.Name.LocalName == "Delete" && a.Attribute("eId").Value == 
    					oFullElement.Elements().Single(e => e.Label() == oSlashGreaterThanTokenLabel).GtID())
    				.Attribute("eLb").Value == oSlashGreaterThanTokenLabel);
    			...
    			if(mFullElement1.Elements().Any(e => e.Label() == mLessThanTokenLabel))
    				mFullElement1.Elements().Single(e => e.Label() == mLessThanTokenLabel).Value = "v0";
    			
    			if(mFullElement1.Elements().Any(e => e.Label() == mSlashGreaterThanTokenLabel))
    				mFullElement1.Elements().Single(e => e.Label() == mSlashGreaterThanTokenLabel).Value = "v1";
    			...
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				...
    				mFullElement.Elements().Single(e => e.Label() == mSlashGreaterThanTokenLabel).GtID());
    			Assert.IsTrue(expander.FullDelta.Actions
    					.Single(a => a.Name.LocalName == "Update" && a.Attribute("eLb").Value == oSlashGreaterThanTokenLabel)
