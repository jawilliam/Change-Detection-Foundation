namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void XmlEmptyElementServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<XmlEmptyElementSyntax> nodes);
         ...
         public void XmlEmptyElementServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<XmlEmptyElementSyntax> nodes = null;
    	    string expectedLabel = null;
    	    XmlEmptyElementServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var LessThanTokenLabel = Enum.GetName(typeof(SyntaxKind), node.LessThanToken.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == LessThanTokenLabel).Attribute("mLb").Value == LessThanTokenLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == LessThanTokenLabel).GtID())
    				.Attribute("mId").Value == mFullElement.GtID());
    
    			var SlashGreaterThanTokenLabel = Enum.GetName(typeof(SyntaxKind), node.SlashGreaterThanToken.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == SlashGreaterThanTokenLabel).Attribute("mLb").Value == SlashGreaterThanTokenLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == SlashGreaterThanTokenLabel).GtID())
    				.Attribute("mId").Value == mFullElement.GtID());
