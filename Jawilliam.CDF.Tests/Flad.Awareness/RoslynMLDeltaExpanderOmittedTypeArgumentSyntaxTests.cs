namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
         /// <summary>
         /// Provides the element revision pair(s) to test in <see cref="OmittedTypeArgumentServiceProvider_RoslynMLDeltaExpander_OK"/>.
         /// </summary>
    	 ...
    	 /// <summary>
         /// Tests expansion logic for <see cref="OmittedTypeArgumentSyntax"/>.
         /// </summary>
    	 [TestMethod]
         public void OmittedTypeArgumentServiceProvider_RoslynMLDeltaExpander_OK()
         {
    		var converter = new CDF.CSharp.RoslynML.RoslynML();
    		var selector = new CDF.CSharp.RoslynML.RoslynMLPruneSelector();
    		int id = 0; 
    
    	    IEnumerable<(OmittedTypeArgumentSyntax, OmittedTypeArgumentSyntax)> nodeRevisionPairs = null;
    	    string oExpectedLabel = null, mExpectedLabel = null;
    	    OmittedTypeArgumentServiceProvider_RoslynMLDeltaExpander_DataToTest(ref nodeRevisionPairs);
    		foreach(((OmittedTypeArgumentSyntax Original, OmittedTypeArgumentSyntax Modified) nodeRevisionPair, Action<RoslynML, XElement> defoliate) in nodeRevisionPairs
    			.SelectMany(n => new List<((OmittedTypeArgumentSyntax, OmittedTypeArgumentSyntax), Action<RoslynML, XElement>)>
    				{ (n, (r, n1) => { }), (n, (r, n1) => r.Defoliate(n1)) }))
    		{
    			...
    			var oOmittedTypeArgumentTokenLabel = Enum.GetName(typeof(SyntaxKind), nodeRevisionPair.Original.OmittedTypeArgumentToken.Kind());
    			var mOmittedTypeArgumentTokenLabel = Enum.GetName(typeof(SyntaxKind), nodeRevisionPair.Modified.OmittedTypeArgumentToken.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == oOmittedTypeArgumentTokenLabel)
    				.Attribute("mLb").Value == mOmittedTypeArgumentTokenLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Label() == oOmittedTypeArgumentTokenLabel).GtID())
    				.Attribute("mId").Value == 
    				mFullElement.Elements().Single(e => e.Label() == mOmittedTypeArgumentTokenLabel).GtID());
    			...
    			Assert.IsTrue(expander.FullDelta.Actions
    					.Single(a => a.Name.LocalName == "Insert" && a.Attribute("eLb").Value == mOmittedTypeArgumentTokenLabel)
    					.Attribute("pLb").Value == mExpectedLabel);
    			Assert.IsTrue(expander.FullDelta.Actions
    			...
    			Assert.IsTrue(expander.FullDelta.Actions
    				.Single(a => a.Name.LocalName == "Delete" && a.Attribute("eId").Value == 
    					oFullElement.Elements().Single(e => e.Label() == oOmittedTypeArgumentTokenLabel).GtID())
    				.Attribute("eLb").Value == oOmittedTypeArgumentTokenLabel);
    			...
    			if(mFullElement1.Elements().Any(e => e.Label() == mOmittedTypeArgumentTokenLabel))
    				mFullElement1.Elements().Single(e => e.Label() == mOmittedTypeArgumentTokenLabel).Value = "v0";
    			...
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				...
    				mFullElement.Elements().Single(e => e.Label() == mOmittedTypeArgumentTokenLabel).GtID());
    			Assert.IsTrue(expander.FullDelta.Actions
    					.Single(a => a.Name.LocalName == "Update" && a.Attribute("eLb").Value == oOmittedTypeArgumentTokenLabel)
