namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void OmittedTypeArgumentServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<OmittedTypeArgumentSyntax> nodes);
         ...
         public void OmittedTypeArgumentServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<OmittedTypeArgumentSyntax> nodes = null;
    	    string expectedLabel = null;
    	    OmittedTypeArgumentServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var OmittedTypeArgumentTokenLabel = Enum.GetName(typeof(SyntaxKind), node.OmittedTypeArgumentToken.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == OmittedTypeArgumentTokenLabel).Attribute("mLb").Value == OmittedTypeArgumentTokenLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == OmittedTypeArgumentTokenLabel).GtID())
