namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void TryStatementServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<TryStatementSyntax> nodes);
         ...
         public void TryStatementServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<TryStatementSyntax> nodes = null;
    	    string expectedLabel = null;
    	    TryStatementServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var TryKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.TryKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == TryKeywordLabel).Attribute("mLb").Value == TryKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == TryKeywordLabel).GtID())
