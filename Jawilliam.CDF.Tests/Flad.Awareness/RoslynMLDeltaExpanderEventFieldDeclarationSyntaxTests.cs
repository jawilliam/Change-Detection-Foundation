namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void EventFieldDeclarationServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<EventFieldDeclarationSyntax> nodes);
         ...
         public void EventFieldDeclarationServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<EventFieldDeclarationSyntax> nodes = null;
    	    string expectedLabel = null;
    	    EventFieldDeclarationServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var EventKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.EventKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == EventKeywordLabel).Attribute("mLb").Value == EventKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == EventKeywordLabel).GtID())
