namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void FinallyClauseServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<FinallyClauseSyntax> nodes);
         ...
         public void FinallyClauseServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<FinallyClauseSyntax> nodes = null;
    	    string expectedLabel = null;
    	    FinallyClauseServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var FinallyKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.FinallyKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == FinallyKeywordLabel).Attribute("mLb").Value == FinallyKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == FinallyKeywordLabel).GtID())
