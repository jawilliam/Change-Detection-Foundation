namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void DefineDirectiveTriviaServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<DefineDirectiveTriviaSyntax> nodes);
         ...
         public void DefineDirectiveTriviaServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<DefineDirectiveTriviaSyntax> nodes = null;
    	    string expectedLabel = null;
    	    DefineDirectiveTriviaServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var DefineKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.DefineKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == DefineKeywordLabel).Attribute("mLb").Value == DefineKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == DefineKeywordLabel).GtID())
    				.Attribute("mId").Value == mFullElement.GtID());
