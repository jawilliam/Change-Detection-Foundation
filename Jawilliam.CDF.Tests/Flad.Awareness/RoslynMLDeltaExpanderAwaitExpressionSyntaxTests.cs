namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
    	 partial void AwaitExpressionServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref IEnumerable<AwaitExpressionSyntax> nodes);
         ...
         public void AwaitExpressionServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_OK()
         {
    	    ...
    	    IEnumerable<AwaitExpressionSyntax> nodes = null;
    	    string expectedLabel = null;
    	    AwaitExpressionServiceProvider_RoslynMLDefoliatedTopologicalAbstraction_DataToTest(ref nodes);
    		foreach(var node in nodes)
    		{
    			...
    			var AwaitKeywordLabel = Enum.GetName(typeof(SyntaxKind), node.AwaitKeyword.Kind());
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oLb").Value == AwaitKeywordLabel).Attribute("mLb").Value == AwaitKeywordLabel);
    			Assert.IsTrue(expander.FullDelta.Matches.Single(m => m.Attribute("oId").Value == 
    				oFullElement.Elements().Single(e => e.Attribute("oLb").Value == AwaitKeywordLabel).GtID())
