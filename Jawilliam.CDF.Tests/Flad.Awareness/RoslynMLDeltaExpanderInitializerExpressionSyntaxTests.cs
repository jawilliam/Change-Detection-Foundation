namespace Jawilliam.CDF.Tests.Flad.Awareness
{
    ...
    {
         /// <summary>
         /// Provides the element revision pair(s) to test in <see cref="InitializerExpressionServiceProvider_RoslynMLDeltaExpander_OK"/>.
         /// </summary>
    	 ...
    	 /// <summary>
         /// Tests expansion logic for <see cref="InitializerExpressionSyntax"/>.
         /// </summary>
    	 [TestMethod]
         public void InitializerExpressionServiceProvider_RoslynMLDeltaExpander_OK()
         {
    		var converter = new CDF.CSharp.RoslynML.RoslynML();
    		var selector = new CDF.CSharp.RoslynML.RoslynMLPruneSelector();
    		int id = 0; 
    
    	    IEnumerable<(InitializerExpressionSyntax, InitializerExpressionSyntax)> nodeRevisionPairs = null;
    	    string oExpectedLabel = null, mExpectedLabel = null;
    	    InitializerExpressionServiceProvider_RoslynMLDeltaExpander_DataToTest(ref nodeRevisionPairs);
    		foreach(((InitializerExpressionSyntax Original, InitializerExpressionSyntax Modified) nodeRevisionPair, Action<RoslynML, XElement> defoliate) in nodeRevisionPairs
    			.SelectMany(n => new List<((InitializerExpressionSyntax, InitializerExpressionSyntax), Action<RoslynML, XElement>)>
    				{ (n, (r, n1) => { }), (n, (r, n1) => r.Defoliate(n1)) }))
    		{
    			...
    			if(mFullElement1.Elements().Any(e => e.Label() == mOpenBraceTokenLabel))
    				mFullElement1.Elements().Single(e => e.Label() == mOpenBraceTokenLabel).Value = "v0";
    			
    			if(mFullElement1.Elements().Any(e => e.Label() == mCloseBraceTokenLabel))
    				mFullElement1.Elements().Single(e => e.Label() == mCloseBraceTokenLabel).Value = "v1";
